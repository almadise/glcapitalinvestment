...
const getSteps = (t: (fr: string, en: string) => string) => [
  ...
  { id: 'step-project', num: 2, label: t('Projet', 'Project'), icon: FileText },
  {
    id: 'step-financing',
    num: 3,
    label: t('Plan de financement', 'Financing Plan'),
    icon: DollarSign,
  },
  { id: 'step-documents', num: 4, label: t('Documents', 'Documents'), icon: Upload },
  {
    id: 'step-review',
    num: 5,
    label: t('Révision & Soumission', 'Review & Submit'),
    icon: CheckCircle2,
  },
];
...
const getDocumentRequirements = (t: (fr: string, en: string) => string): DocumentRequirement[] => [
  {
    ...
    required: true,
    desc: t('PDF, 2-5 pages max', 'PDF, 2-5 pages max'),
  },
  {
    id: 'business_plan',
    label: t('Business plan / Etude de faisabilite', 'Business plan / Feasibility study'),
    required: true,
    desc: t('PDF ou DOCX', 'PDF or DOCX'),
  },
  {
    id: 'kyc_corporate',
    label: t('KYC corporate', 'Corporate KYC'),
    required: true,
    desc: t('PDF: registre, statuts, UBO', 'PDF: registry, articles, UBO'),
  },
  {
    id: 'financial_model',
    label: t('Modele de financement du projet', 'Project financing model'),
    required: false,
    desc: t('Excel (.xlsx)', 'Excel (.xlsx)'),
  },
  {
    id: 'source_of_funds',
    label: t("Justificatif d'origine des fonds", 'Source of funds proof'),
    required: false,
    desc: t("Releves bancaires ou rapports d'audit", 'Bank statements or audit reports'),
  },
  {
    id: 'key_contracts',
    label: t('Contrats cles (Off-take / EPC)', 'Key contracts (Off-take / EPC)'),
    required: false,
    desc: t('Si disponible', 'If available'),
  },
];
...
export default function WizardContent() {
  ...
  const getDocLabel = useCallback(
    (docTypeId: string) =>
      documentRequirements.find((doc) => doc.id === docTypeId)?.label || docTypeId,
  ...
  const processFiles = async (files: File[]) => {
    ...
    for (const file of files) {
      if (!allowed.includes(file.type)) {
        toast.error(
          t(
            `${file.name} : Type de fichier non autorisé. PDF, DOCX, XLSX uniquement.`,
            `${file.name}: File type not allowed. PDF, DOCX, XLSX only.`
          )
        );
        continue;
      }
      if (file.size > 25 * 1024 * 1024) {
        toast.error(
          t(
            `${file.name} : Le fichier dépasse la limite de 25 Mo`,
            `${file.name}: File exceeds 25MB limit`
          )
        );
        continue;
      }
      ...
      try {
        ...
        toast.success(
          t(
            `${file.name} - téléchargé et scan antivirus réussi`,
            `${file.name} - uploaded & antivirus scan passed`
          )
        );
        ...
        toast.error(
          t(
            `${file.name} : Échec du téléchargement - ${err.message}`,
            `${file.name}: Upload failed - ${err.message}`
          )
        );
      }
    }
  ...
  const handleFinalSubmit = async () => {
    if (!user) {
      toast.error(
        t(
          'Vous devez être connecté pour soumettre un dossier',
    ...
    await supabase.from('compliance_logs').insert({
      ...
      severity: complianceResult.blocked
        ? 'critical'
    ...
    try {
      const optionalDocCount = cleanFiles.filter(
        (file) => !requiredDocIds.includes(file.docType)
      ).length;
      ...
      const { data, removedColumns } = await insertCaseFileWithSchemaFallback<{
        ref?: string;
        id?: string;
      }>({
        supabase,
      ...
      const ref =
        data?.ref ||
        `GLC-${new Date().getFullYear()}-${String(Math.floor(Math.random() * 9000) + 1000)}`;
      ...
      setSubmitted(true);
      toast.success(
        t(`Dossier soumis - Référence : ${ref}`, `Application submitted - Reference: ${ref}`)
      );
    } catch (err: any) {
      setIsSubmitting(false);
      toast.error(
        err.message ||
          t('Échec de la soumission. Veuillez réessayer.', 'Submission failed. Please try again.')
      );
    }
  };
    ...
    return (
      <div className="max-w-2xl mx-auto text-center py-16">
            ...
            <div className="flex items-start gap-2">
              ...
              <div>
                <p className="text-emerald-800 text-xs font-semibold mb-1">
                  {uploadedFiles.filter((f) => f.status === 'clean').length}{' '}
                  {t('document(s) stocké(s) en sécurité', 'document(s) securely stored')}
                </p>
          ...
          <div className="flex items-start gap-2">
            ...
            <div>
              <p className="text-amber-800 text-xs font-semibold mb-1">
                {t('Prochaines étapes', 'What happens next?')}
              </p>
              <ul className="text-amber-700 text-xs space-y-1 leading-relaxed">
                <li>
                  •{' '}
                  {t(
                    'Notre équipe examinera votre dossier sous 1 à 3 jours ouvrables',
                ...
                <li>
                  •{' '}
                  {t(
                    'Vous pourrez être contacté pour des documents supplémentaires',
                ...
                <li>
                  •{' '}
                  {t(
                    'Toutes les communications se feront via le portail sécurisé',
                    'All communications will occur through the secure portal'
                  )}
                </li>
                <li>
                  •{' '}
                  {t(
                    'Les soumissions aux partenaires sont confidentielles sous NCNDA',
                    'Partner submissions are confidential under NCNDA'
                  )}
                </li>
              </ul>
            </div>
        ...
        <a
          ...
          className="inline-flex items-center gap-2 px-6 py-3 bg-navy hover:bg-navy/90 text-white font-semibold rounded-xl transition-all duration-200 active:scale-95"
        >
          ...
          {steps.map((step, i) => (
            <React.Fragment key={step.id}>
              ...
              >
                ...
                >
                  {step.num < currentStep ? <CheckCircle2 size={18} /> : <step.icon size={16} />}
                </div>
                <span
          ...
          <div className="p-8">
            <div className="flex items-center gap-3 mb-6">
              ...
              <div>
                ...
                <p className="text-gray-500 text-sm">
                  {t(
                    "Détails de l'entité juridique et propriété effective",
                    'Legal entity details and beneficial ownership'
                  )}
                </p>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              <div className="lg:col-span-2">
                <h3 className="text-sm font-bold text-navy mb-4 pb-2 border-b border-gray-100">
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1.5">
                  {t('Dénomination sociale', 'Legal Name of Organization')}{' '}
                  <span className="text-red-500">*</span>
                </label>
                <p className="text-xs text-gray-400 mb-2">
                  {t(
                    "Telle qu'enregistrée auprès de l'autorité compétente",
                    'As registered with the relevant authority'
                  )}
                </p>
                <input
                  {...identityForm.register('orgName', {
                    required: t("Nom de l'organisation requis", 'Organization name is required'),
                  })}
                ...
                {identityForm.formState.errors.orgName && (
                  <p className="text-red-500 text-xs mt-1 flex items-center gap-1">
                    <AlertCircle size={11} />
                    {identityForm.formState.errors.orgName.message}
                  </p>
                )}
              </div>
              ...
              <div>
                ...
                <p className="text-xs text-gray-400 mb-2">
                  {t(
                    "Juridiction où l'entité est enregistrée",
                ...
                {identityForm.formState.errors.orgCountry && (
                  <p className="text-red-500 text-xs mt-1 flex items-center gap-1">
                    <AlertCircle size={11} />
                    {identityForm.formState.errors.orgCountry.message}
                  </p>
                )}
              </div>
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1.5">
                  {t('Numéro de registre / SIRET', 'Registry / Company Number')}{' '}
                  <span className="text-red-500">*</span>
                </label>
                <input
                  {...identityForm.register('registryNumber', {
                    required: t('Numéro de registre requis', 'Registry number is required'),
                  })}
                ...
                {identityForm.formState.errors.registryNumber && (
                  <p className="text-red-500 text-xs mt-1 flex items-center gap-1">
                    <AlertCircle size={11} />
                    {identityForm.formState.errors.registryNumber.message}
                  </p>
                )}
              </div>
              ...
              <div>
                ...
                <select
                  {...identityForm.register('orgType', {
                    required: t("Type d'entité requis", 'Entity type is required'),
                  })}
                  className="w-full px-4 py-2.5 border border-gray-200 rounded-xl text-sm focus:outline-none focus:border-navy transition-colors bg-white"
                >
                  <option value="">{t('Sélectionner un type...', 'Select type...')}</option>
                  <option value="llc">
                    {t('SARL / LLC', 'Limited Liability Company (LLC / SARL)')}
                  </option>
                  <option value="plc">{t('SA / PLC', 'Public Limited Company (PLC / SA)')}</option>
                  <option value="spv">
                    {t('Véhicule ad hoc (SPV)', 'Special Purpose Vehicle (SPV)')}
                  </option>
                  <option value="holding">{t('Holding', 'Holding Company')}</option>
                  <option value="partnership">
                    {t('Société de personnes / LLP', 'Partnership / LLP')}
                  </option>
                  <option value="individual">
                    {t('Personne physique / Auto-entrepreneur', 'Individual / Sole Proprietor')}
                  </option>
                </select>
                {identityForm.formState.errors.orgType && (
                  <p className="text-red-500 text-xs mt-1 flex items-center gap-1">
                    <AlertCircle size={11} />
                    {identityForm.formState.errors.orgType.message}
                  </p>
                )}
              </div>
              ...
              <div className="lg:col-span-2 mt-2">
                <h3 className="text-sm font-bold text-navy mb-4 pb-2 border-b border-gray-100 flex items-center gap-2">
                  ...
                  <span className="text-xs font-normal text-gray-400">
                    {t(
                      'Requis pour les entités avec participation > 25%',
                      'Required for entities with ownership > 25%'
                    )}
                  </span>
                </h3>
              </div>
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1.5">
                  {t("Nom complet de l'UBO", 'UBO Full Name')}
                </label>
                <input
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1.5">
                  {t("Nationalité de l'UBO", 'UBO Nationality')}
                </label>
                <input
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1.5">
                  {t('Pourcentage de détention', 'Ownership Percentage')}
                </label>
                <input
              ...
              <div>
                ...
                <input
                  {...identityForm.register('contactName', {
                    required: t('Nom du contact requis', 'Contact name is required'),
                  })}
                ...
                {identityForm.formState.errors.contactName && (
                  <p className="text-red-500 text-xs mt-1 flex items-center gap-1">
                    <AlertCircle size={11} />
                    {identityForm.formState.errors.contactName.message}
                  </p>
                )}
              </div>
          ...
          <div className="p-8">
            <div className="flex items-center gap-3 mb-6">
              ...
              <div>
                ...
                <p className="text-gray-500 text-sm">
                  {t(
                    'Décrivez le projet ou la transaction nécessitant un financement',
                    'Describe the project or transaction requiring financing'
                  )}
                </p>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              <div>
                ...
                <p className="text-xs text-gray-400 mb-2">
                  {t(
                    'Sélectionnez la catégorie de financement principale',
                    'Select the primary financing category'
                  )}
                </p>
                <select
                  {...projectForm.register('requestType', {
                    required: t('Type de demande requis', 'Request type is required'),
                  })}
                  className="w-full px-4 py-2.5 border border-gray-200 rounded-xl text-sm focus:outline-none focus:border-navy transition-colors bg-white"
                >
                  <option value="">{t('Sélectionner un type...', 'Select type...')}</option>
                  ...
                  <option value="advisory">
                    {t('Conseil & Structuration uniquement', 'Advisory & Structuring Only')}
                  </option>
                  <option value="trade-finance">
                    {t('Financement du commerce', 'Trade Finance')}
                  </option>
                  <option value="other">{t('Autre', 'Other')}</option>
                </select>
                {projectForm.formState.errors.requestType && (
                  <p className="text-red-500 text-xs mt-1 flex items-center gap-1">
                    <AlertCircle size={11} />
                    {projectForm.formState.errors.requestType.message}
                  </p>
                )}
              </div>
              ...
              <div>
                ...
                <input
                  {...projectForm.register('projectName', {
                    required: t('Nom du projet requis', 'Project name is required'),
                  })}
                ...
                {projectForm.formState.errors.projectName && (
                  <p className="text-red-500 text-xs mt-1 flex items-center gap-1">
                    <AlertCircle size={11} />
                    {projectForm.formState.errors.projectName.message}
                  </p>
                )}
              </div>
              ...
              <div>
                ...
                <select
                  {...projectForm.register('sector', {
                    required: t('Secteur requis', 'Sector is required'),
                  })}
                  className="w-full px-4 py-2.5 border border-gray-200 rounded-xl text-sm focus:outline-none focus:border-navy transition-colors bg-white"
                >
                  <option value="">{t('Sélectionner un secteur...', 'Select sector...')}</option>
                  <option value="energy">
                    {t('Énergie & Renouvelables', 'Energy & Renewables')}
                  </option>
                  <option value="infrastructure">
                    {t('Infrastructure & Transport', 'Infrastructure & Transport')}
                  </option>
                  <option value="real-estate">
                    {t('Immobilier & Construction', 'Real Estate & Construction')}
                  </option>
                  ...
                  <option value="telecom">
                    {t('Télécom & Technologie', 'Telecom & Technology')}
                  </option>
                  <option value="finance">{t('Services financiers', 'Financial Services')}</option>
                  <option value="trade">
                    {t('Commerce & Matières premières', 'Trade & Commodities')}
                  </option>
                  <option value="other">{t('Autre', 'Other')}</option>
                </select>
                {projectForm.formState.errors.sector && (
                  <p className="text-red-500 text-xs mt-1 flex items-center gap-1">
                    <AlertCircle size={11} />
                    {projectForm.formState.errors.sector.message}
                  </p>
                )}
              </div>
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1.5">
                  {t('Budget total du projet', 'Total Project Budget')}{' '}
                  <span className="text-red-500">*</span>
                </label>
                <p className="text-xs text-gray-400 mb-2">
                  {t(
                    'Capex total approximatif incluant les imprévus',
                    'Approximate total capex including contingencies'
                  )}
                </p>
                <input
                  {...projectForm.register('totalBudget', {
                    required: t('Budget requis', 'Budget is required'),
                  })}
                ...
                {projectForm.formState.errors.totalBudget && (
                  <p className="text-red-500 text-xs mt-1 flex items-center gap-1">
                    <AlertCircle size={11} />
                    {projectForm.formState.errors.totalBudget.message}
                  </p>
                )}
              </div>
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1.5">
                  {t('Date de démarrage prévue', 'Expected Start Date')}
                </label>
                <input
                  type="date"
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1.5">
                  {t(
                    "Chiffre d'affaires annuel attendu (si applicable)",
                    'Expected Annual Revenue (if applicable)'
                  )}
                </label>
                <input
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1.5">
                  {t('Sponsors du projet', 'Project Sponsors')}
                </label>
                <p className="text-xs text-gray-400 mb-2">
                  {t(
                    'Noms des principaux sponsors ou investisseurs en fonds propres',
                    'Names of key sponsors or equity investors'
                  )}
                </p>
                <input
              ...
              <div className="lg:col-span-2">
                ...
                <p className="text-xs text-gray-400 mb-2">
                  {t(
                    'Brève description du projet, ses objectifs et sa justification stratégique (min. 100 caractères)',
                    'Brief description of the project, its objectives, and strategic rationale (min. 100 characters)'
                  )}
                </p>
                <textarea
                  ...
                  rows={5}
                  placeholder={t(
                    'Décrivez votre projet : objectifs, périmètre, localisation, impact attendu et raisons du financement...',
                    'Describe your project: objectives, scope, location, expected impact, and why financing is being sought...'
                  )}
                ...
                {projectForm.formState.errors.projectDescription && (
                  <p className="text-red-500 text-xs mt-1 flex items-center gap-1">
                    <AlertCircle size={11} />
                    {projectForm.formState.errors.projectDescription.message}
                  </p>
                )}
              </div>
            </div>
          ...
          <div className="p-8">
            <div className="flex items-center gap-3 mb-6">
              ...
              <div>
                ...
                <p className="text-gray-500 text-sm">
                  {t(
                    'Définissez la structure dette/fonds propres, les garanties et la traçabilité des fonds',
                    'Define the debt/equity structure, guarantees, and fund traceability'
                  )}
                </p>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1.5">
                  {t('Montant de dette recherché', 'Debt Amount Sought')}{' '}
                  <span className="text-red-500">*</span>
                </label>
                <p className="text-xs text-gray-400 mb-2">
                  {t(
                    'Dette senior, mezzanine ou montant total du prêt',
                    'Senior debt, mezzanine, or total loan amount'
                  )}
                </p>
                <input
                  {...financingForm.register('debtAmount', {
                    required: t('Montant de dette requis', 'Debt amount is required'),
                  })}
                ...
                {financingForm.formState.errors.debtAmount && (
                  <p className="text-red-500 text-xs mt-1 flex items-center gap-1">
                    <AlertCircle size={11} />
                    {financingForm.formState.errors.debtAmount.message}
                  </p>
                )}
              </div>
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1.5">
                  {t('Fonds propres / Apport personnel', 'Equity / Own Contribution')}
                </label>
                <p className="text-xs text-gray-400 mb-2">
                  {t('Fonds propres engagés par les sponsors', 'Equity committed by sponsors')}
                </p>
                <input
              ...
              <div>
                ...
                <select
                  {...financingForm.register('maturity', {
                    required: t('Maturité requise', 'Maturity is required'),
                  })}
                  className="w-full px-4 py-2.5 border border-gray-200 rounded-xl text-sm focus:outline-none focus:border-navy transition-colors bg-white"
                >
                  <option value="">
                    {t('Sélectionner une maturité...', 'Select maturity...')}
                  </option>
                ...
                {financingForm.formState.errors.maturity && (
                  <p className="text-red-500 text-xs mt-1 flex items-center gap-1">
                    <AlertCircle size={11} />
                    {financingForm.formState.errors.maturity.message}
                  </p>
                )}
              </div>
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1.5">
                  {t('Type de garantie disponible', 'Available Guarantee Type')}
                </label>
                <select
                ...
                >
                  <option value="">
                    {t('Sélectionner une garantie...', 'Select guarantee...')}
                  </option>
                  <option value="real-estate">
                    {t('Immobilier / Hypothèque', 'Real Estate / Mortgage')}
                  </option>
                  <option value="equipment">
                    {t('Équipements / Actifs', 'Equipment / Assets')}
                  </option>
                  <option value="receivables">
                    {t('Créances / Off-take', 'Receivables / Off-take')}
                  </option>
                  <option value="personal">
                    {t('Garantie personnelle', 'Personal Guarantee')}
                  </option>
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1.5">
                  {t('Origine des fonds / Fonds propres', 'Source of Funds / Equity')}{' '}
                  <span className="text-red-500">*</span>
                </label>
                <p className="text-xs text-gray-400 mb-2">
                  {t(
                    "Décrivez l'origine de l'apport en fonds propres et la traçabilité des fonds",
                    'Describe origin of equity contribution and fund traceability'
                  )}
                </p>
                <select
                  {...financingForm.register('fundSource', {
                    required: t('Origine des fonds requise', 'Source of funds is required'),
                  })}
                  className="w-full px-4 py-2.5 border border-gray-200 rounded-xl text-sm focus:outline-none focus:border-navy transition-colors bg-white"
                >
                  ...
                  <option value="operating">
                    {t(
                      "Bénéfices d'exploitation / Réserves",
                      'Operating profits / retained earnings'
                    )}
                  </option>
                  <option value="sale">
                    {t("Produit de cession d'actifs", 'Asset sale proceeds')}
                  </option>
                  <option value="investment">
                    {t("Capital d'investisseurs existants", 'Existing investor capital')}
                  </option>
                  <option value="grant">
                    {t('Subvention / Financement public', 'Grant / public funding')}
                  </option>
                  <option value="other">
                    {t('Autre (préciser dans les notes)', 'Other (specify in notes)')}
                  </option>
                </select>
                {financingForm.formState.errors.fundSource && (
                  <p className="text-red-500 text-xs mt-1 flex items-center gap-1">
                    <AlertCircle size={11} />
                    {financingForm.formState.errors.fundSource.message}
                  </p>
                )}
              </div>
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1.5">
                  {t('Financements existants', 'Existing Financing Arrangements')}
                </label>
                <input
                  {...financingForm.register('existingFinancing')}
                  placeholder={t(
                    'ex. Prêt senior IFC 5M€ (actif), BOAD subordonné 3M€',
                    'e.g. IFC senior loan €5M (active), BOAD subordinated €3M'
                  )}
                  className="w-full px-4 py-2.5 border border-gray-200 rounded-xl text-sm focus:outline-none focus:border-navy transition-colors"
                />
              </div>
              ...
              <div className="lg:col-span-2">
                <label className="block text-sm font-medium text-gray-700 mb-1.5">
                  {t("Type d'institution préféré", 'Preferred Institution Type')}
                </label>
                <p className="text-xs text-gray-400 mb-2">
                  {t(
                    'Indiquez votre préférence - GL Capital évaluera la faisabilité et orientera en conséquence',
                    'Indicate preference - GL Capital will assess feasibility and route accordingly'
                  )}
                </p>
                <input
                  {...financingForm.register('targetInstitution')}
                  placeholder={t(
                    'ex. Institution de financement du développement (IFD), banque commerciale, fonds spécialisé',
                    'e.g. Development Finance Institution (DFI), commercial bank, specialized fund'
                  )}
                  className="w-full px-4 py-2.5 border border-gray-200 rounded-xl text-sm focus:outline-none focus:border-navy transition-colors"
                />
              </div>
              ...
              <div className="lg:col-span-2">
                <label className="block text-sm font-medium text-gray-700 mb-1.5">
                  {t('Notes complémentaires', 'Additional Notes')}
                </label>
                <textarea
                  {...financingForm.register('additionalNotes')}
                  rows={3}
                  placeholder={t(
                    'Tout contexte supplémentaire : conditions particulières, urgence, rejets antérieurs, exigences spécifiques...',
                    'Any additional context: special conditions, urgency, previous rejections, specific requirements...'
                  )}
                  className="w-full px-4 py-3 border border-gray-200 rounded-xl text-sm focus:outline-none focus:border-navy transition-colors resize-none"
                />
              </div>
        ...
        {currentStep === 4 && (
          <div className="p-8">
            <div className="flex items-center gap-3 mb-6">
              ...
              <div>
                <h2 className="text-lg font-bold text-navy">
                  {t('Téléchargement de documents', 'Document Uploads')}
                </h2>
                <p className="text-gray-500 text-sm">
                  {t(
                    'Téléchargez les documents requis. Tous les fichiers sont chiffrés, scannés et stockés en sécurité.',
                    'Upload required documents. All files are encrypted, antivirus-scanned, and stored securely.'
                  )}
                </p>
              </div>
            </div>
              ...
              <div>
                ...
                >
                  ...
                  <p className="text-gray-400 text-xs mb-4">
                    PDF, DOCX, XLSX · {t('Max 25 Mo par fichier', 'Max 25MB per file')}
                  </p>
                ...
                <div className="space-y-2 mb-6">
                  {documentRequirements.map((dt) => {
                    const uploaded = uploadedFiles.filter(
                      (f) => f.docType === dt.id && f.status === 'clean'
                    );
                    return (
                      <div
                        key={dt.id}
                      ...
                      >
                        ...
                        <div className="flex-1 min-w-0">
                          <p
                            className={`text-xs font-semibold ${uploaded.length > 0 ? 'text-emerald-700' : 'text-gray-700'}`}
                          >
                            {dt.label}
        ...
        {currentStep === 5 && (
          <div className="p-8">
            <div className="flex items-center gap-3 mb-6">
              ...
              <div>
                <h2 className="text-lg font-bold text-navy">
                  {t('Révision & Soumission', 'Review & Submit')}
                </h2>
                <p className="text-gray-500 text-sm">
                  {t(
                ...
                <dl className="grid grid-cols-2 gap-x-6 gap-y-2">
                  {[
                    ...
                    <div key={`review-id-${item.label}`}>
                      <dt className="text-[10px] text-gray-400 uppercase tracking-wider">
                        {item.label}
                      </dt>
                      <dd className="text-xs font-semibold text-navy mt-0.5">
                        {item.value || '-'}
                      </dd>
                    </div>
                  ))}
                ...
                <dl className="grid grid-cols-2 gap-x-6 gap-y-2">
                  {[
                    ...
                    { label: t('Pays', 'Country'), value: projectValues.country },
                    {
                      ...
                      value: `${projectValues.currency || 'EUR'} ${projectValues.totalBudget}`,
                    },
                  ].map((item) => (
              ...
              <div className="bg-gray-50 rounded-2xl p-5 border border-gray-200">
                ...
                <dl className="grid grid-cols-2 gap-x-6 gap-y-2">
                  {[
                    ...
                    {
                      label: t('Origine des fonds', 'Fund Source'),
                      value: financingValues.fundSource,
                    },
                  ].map((item) => (
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    <p className="text-white text-xs font-semibold mb-1">
                      {t('Déclaration de conformité', 'Compliance Declaration')}
                    </p>
          ...
          {currentStep < 5 ? (
            ...
            >
              {isSubmitting ? (
                ...
                <>
                  {t('Soumettre le dossier', 'Submit application')}
