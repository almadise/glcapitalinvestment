...
<title>${isFr ? 'Dossier reçu - GL Capital' : 'Application received - GL Capital'}</title></head>
      ...
      <table width="620" cellpadding="0" cellspacing="0" style="max-width:620px;width:100%;">
        <tr><td style="background:linear-gradient(135deg,#0a1941 0%,#0d2060 100%);border-radius:12px 12px 0 0;padding:36px 40px;text-align:center;">
          ...
          <div style="margin-top:20px;display:inline-block;background:rgba(201,168,76,0.12);border:1px solid rgba(201,168,76,0.3);border-radius:20px;padding:6px 18px;">
            <span style="color:#c9a84c;font-size:12px;font-weight:600;letter-spacing:1px;text-transform:uppercase;">📁 ${isFr ? 'Dossier reçu' : 'Application received'}</span>
          </div>
        </td></tr>
        ...
        <tr><td style="background:#ffffff;padding:40px 40px 0;">
          ...
          <p style="margin:0 0 28px;color:#475569;font-size:14px;line-height:1.7;">${
            isFr
              ...
              : 'We have received your financing application. Our team will proceed with its analysis as soon as possible. You will be notified of each update.'
          }</p>
        </td></tr>
        ...
        <tr><td style="background:#ffffff;padding:0 40px 36px;text-align:center;">
          <a href="${portalLink}" style="display:inline-block;background:#c9a84c;color:#0a1941;font-size:13px;font-weight:700;letter-spacing:1px;text-transform:uppercase;padding:14px 32px;border-radius:8px;text-decoration:none;">${isFr ? 'Suivre mon dossier' : 'Track my application'}</a>
        </td></tr>
        <tr><td style="background:#0a1941;border-radius:0 0 12px 12px;padding:24px 40px;text-align:center;">
          ...
          <p style="margin:0;color:#475569;font-size:11px;">${isFr ? 'Confirmation automatique de réception de dossier.' : 'Automatic application receipt confirmation.'}</p>
        </td></tr>
      </table>
    </td></tr>
  </table>
...
export async function POST(req: NextRequest) {
  ...
  const subject = isFr
    ...
    : `Application received - ${caseTitle} | GL Capital`;
