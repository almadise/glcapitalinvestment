...
export default function ComplianceCasesPage() {
  ...
  const [activeTab, setActiveTab] = useState<'decision' | 'documents' | 'history' | 'tags'>(
    'decision'
  );
    ...
    try {
      ...
      if (user?.id) {
        await logAuditAction({
          ...
          reason:
            statusNote.trim() || (newStatus === 'REJETE' ? rejectionReason.trim() : undefined),
          metadata: {
            old_status: selectedCase.status,
            new_status: newStatus,
            portal: 'compliance',
    ...
    try {
      ...
      if (user?.id) {
        await logAuditAction({
          ...
          actorId: user.id,
          actorEmail: user.email || undefined,
          metadata: { case_title: caseFileLabel(selectedCase), portal: 'compliance' },
        });
      }
    ...
    <ComplianceLayout>
      <div className="mb-4 sm:mb-6 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div>
          ...
          <p className="text-slate-500 text-sm">
            Validation conformité, décisions et règles KYC/AML
          </p>
        </div>
        <button
          onClick={fetchCases}
          className="flex items-center gap-2 px-3 py-2 bg-white border border-slate-200 rounded-lg text-sm text-slate-600 hover:bg-slate-50 self-start sm:self-auto"
        >
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-4">
        {(['EN_REVUE_COMPLIANCE', 'ELIGIBLE', 'REJETE', 'ALL'] as const).map((s) => {
          ...
          const cfg =
            s === 'ALL'
              ? { label: 'Total', color: 'text-slate-600', bg: 'bg-slate-50' }
              : {
          ...
          return (
            <button
              ...
              className={`p-3 rounded-xl border text-left transition-all ${filters.status === s ? 'border-emerald-400 ring-2 ring-emerald-200' : 'border-slate-200 hover:border-emerald-200'} bg-white`}
            >
              ...
              {(['decision', 'documents', 'history', 'tags'] as const).map((tab) => (
                ...
                >
                  {tab === 'decision' ? (
                    ...
                    `Historique${statusHistory.length > 0 ? ` (${statusHistory.length})` : ''}`
                  ) : (
              ...
              {detailLoading ? (
                ...
                <div className="space-y-4">
                        ...
                        {newStatus && (
                          <div className="space-y-2">
                            <textarea
                              ...
                              placeholder="Note de décision KYC/AML (optionnel)..."
                              rows={2}
                            ...
                            {newStatus === 'REJETE' && (
                              <textarea
                                ...
                                placeholder="Motif de rejet KYC/AML (obligatoire)..."
                                rows={2}
                            ...
                            <button
                              ...
                              className="flex items-center gap-2 px-4 py-2 bg-emerald-600 text-white rounded-lg text-sm font-semibold hover:bg-emerald-700 disabled:opacity-50 transition-colors"
                            >
                              {updatingStatus ? (
                  ...
                  <div className="border-t border-slate-100 pt-4">
                    <h3 className="font-semibold text-navy text-sm mb-3 flex items-center gap-1.5">
                      <Plus size={13} /> Note interne
                    </h3>
                    <div className="flex gap-2">
                      <textarea
                        ...
                        rows={3}
                        className="flex-1 px-3 py-2 border border-slate-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500/20 resize-none"
                      />
                      <button
                        onClick={handleAddNote}
                        disabled={addingNote || !noteContent.trim()}
                        className="self-end px-3 py-2 bg-emerald-600 text-white rounded-lg hover:bg-emerald-700 disabled:opacity-50 transition-colors"
                      >
                        {addingNote ? (
                          <Loader2 size={14} className="animate-spin" />
                        ) : (
                ...
                <div className="space-y-3">
                  <div className="bg-emerald-50 border border-emerald-200 rounded-lg p-3 text-xs text-emerald-700">
                    <strong>Documents KYC/AML :</strong> Téléversez les pièces justificatives,
                    documents de conformité ou tout fichier lié à ce dossier.
                  </div>
