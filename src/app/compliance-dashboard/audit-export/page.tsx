...
export default function ComplianceAuditExportPage() {
  ...
  const exportCSV = () => {
    ...
    let csvContent =
      exportType === 'history'
        ? 'ID,Dossier ID,Ancien statut,Nouveau statut,Modifié par,Note,Date\n'
    ...
    <ComplianceLayout>
      <div className="mb-6">
        ...
        <p className="text-slate-500 text-sm">
          Historique des décisions et notes internes - accès Compliance uniquement
        </p>
      </div>
      ...
      <div className="bg-red-50 border border-red-200 rounded-xl p-3 flex items-center gap-3 mb-6 text-xs text-red-800">
        ...
        <span>
          <strong>Accès restreint :</strong> Ces exports contiennent des données sensibles de
          conformité.
        </span>
      </div>
          ...
          <div>
            ...
            <div className="flex gap-2">
              <button
                ...
                className={`px-3 py-1.5 rounded-lg text-xs font-semibold border transition-all ${exportType === 'history' ? 'bg-emerald-600 text-white border-emerald-600' : 'bg-white text-slate-600 border-slate-200 hover:bg-slate-50'}`}
              >
                Historique statuts
              </button>
              <button
                onClick={() => setExportType('notes')}
                className={`px-3 py-1.5 rounded-lg text-xs font-semibold border transition-all ${exportType === 'notes' ? 'bg-emerald-600 text-white border-emerald-600' : 'bg-white text-slate-600 border-slate-200 hover:bg-slate-50'}`}
              >
                Notes internes
              </button>
            </div>
