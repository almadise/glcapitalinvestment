...
export default function ComplianceAuditExportPage() {
  ...
  const exportCSV = () => {
    ...
    let csvContent = exportType === 'history' ?'ID,Dossier ID,Ancien statut,Nouveau statut,Modifié par,Note,Date\n' :'ID,Dossier ID,Auteur,Contenu,Date\n';
    ...
    a.href = url;
    a.download = `audit_compliance_${exportType}_${new Date().toISOString().split('T')[0]}.csv`;
    a.click();
  ...
  const exportPDF = () => {
    ...
    const a = document.createElement('a');
    a.href = url;
    a.download = `audit_compliance_${exportType}_${new Date().toISOString().split('T')[0]}.html`;
    a.click();
    URL.revokeObjectURL(url);
    toast.success('Export PDF (HTML) téléchargé');
  };
  ...
  return (
    <ComplianceLayout>
      ...
      <div className="bg-red-50 border border-red-200 rounded-xl p-3 flex items-center gap-3 mb-6 text-xs text-red-800">
        ...
        <span><strong>Accès restreint :</strong> Ces exports contiennent des données sensibles de conformité.</span>
      </div>
        ...
        <div className="flex flex-wrap items-end gap-4">
          <div>
            ...
            <div className="flex gap-2">
              <button onClick={() => setExportType('history')} className={`px-3 py-1.5 rounded-lg text-xs font-semibold border transition-all ${exportType === 'history' ? 'bg-emerald-600 text-white border-emerald-600' : 'bg-white text-slate-600 border-slate-200 hover:bg-slate-50'}`}>Historique statuts</button>
              <button onClick={() => setExportType('notes')} className={`px-3 py-1.5 rounded-lg text-xs font-semibold border transition-all ${exportType === 'notes' ? 'bg-emerald-600 text-white border-emerald-600' : 'bg-white text-slate-600 border-slate-200 hover:bg-slate-50'}`}>Notes internes</button>
            </div>
          </div>
          <div>
          ...
          <button onClick={fetchData} className="flex items-center gap-2 px-3 py-2 bg-slate-100 text-slate-700 rounded-lg text-sm hover:bg-slate-200"><Filter size={14} /> Filtrer</button>
          <div className="flex gap-2 ml-auto">
            <button onClick={exportCSV} className="flex items-center gap-2 px-4 py-2 bg-emerald-600 text-white rounded-lg text-sm font-semibold hover:bg-emerald-700"><Download size={14} /> CSV</button>
            <button onClick={exportPDF} className="flex items-center gap-2 px-4 py-2 bg-navy text-white rounded-lg text-sm font-semibold hover:bg-navy/90"><FileText size={14} /> PDF</button>
          </div>
        </div>
      </div>
            ...
            <table className="w-full text-xs">
              ...
              <tbody className="divide-y divide-slate-100">
                {displayData.length === 0 ? (
                  <tr><td colSpan={5} className="text-center py-10 text-slate-400">Aucune donnée</td></tr>
                ) : exportType === 'history' ? (
