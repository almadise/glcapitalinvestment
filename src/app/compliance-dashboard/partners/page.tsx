...
export default function CompliancePartnersPage() {
  ...
  const fetchPartners = useCallback(async () => {
    ...
    setError(null);
    try {
      const { data, error: fetchError } = await supabase.from('partners').select('*').eq('is_active', true).order('name');
  ...
  return (
    <ComplianceLayout>
      ...
      <div className="bg-amber-50 border border-amber-200 rounded-xl p-3 flex items-center gap-3 mb-4 text-xs text-amber-800">
        ...
        <span><strong>Confidentiel :</strong> Ces informations ne sont pas visibles côté portail client. Le client voit uniquement "dossier soumis à une institution agréée".</span>
      </div>

      <div className="bg-white rounded-xl border border-slate-200 p-4 mb-4">
        <div className="relative">
          <Search size={15} className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400" />
          <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Rechercher un partenaire..." className="w-full pl-9 pr-3 py-2 border border-slate-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500/20" />
        </div>
      </div>
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
          {filtered.length === 0 ? (
            ...
            return (
              <div key={p.id} className="bg-white rounded-xl border border-slate-200 p-5 hover:border-emerald-200 hover:shadow-md transition-all">
                <div className="mb-3">
                ...
                {p.zones && p.zones.length > 0 && (
                  <div className="flex items-center gap-1.5 text-xs text-slate-500 mb-2"><Globe size={11} /><span>{p.zones.join(', ')}</span></div>
                )}
                {p.criteria && <p className="text-xs text-slate-600 mb-2 line-clamp-2">{p.criteria}</p>}
                {p.internal_contact && <div className="text-xs text-slate-500 mb-1"><span className="font-medium">Contact interne :</span> {p.internal_contact}</div>}
                {p.contact_email && (
                  <div className="flex items-center gap-1.5 text-xs text-slate-500"><Mail size={11} />{p.contact_email}</div>
                )}
              </div>
            );
          })}
