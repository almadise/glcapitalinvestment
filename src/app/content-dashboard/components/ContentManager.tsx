...
export function ContentManager({ type, title, icon: IconComp }: ContentManagerProps) {
  ...
  const handleSave = async () => {
    if (!form.title_fr.trim() || !form.slug.trim()) {
  ...
  const handleToggleStatus = async (item: ContentPage) => {
    const nextStatus =
      item.status === 'brouillon' ? 'publie' : item.status === 'publie' ? 'archive' : 'brouillon';
    try {
      await supabase
        .from('content_pages')
  ...
  const filtered = items.filter(
    (i) =>
      !search ||
      i.title_fr.toLowerCase().includes(search.toLowerCase()) ||
      ...
      <div className="mb-6 flex items-center justify-between">
        <div>
          ...
          <p className="text-slate-500 text-sm">
            Créer, modifier et publier du contenu institutionnel
          </p>
        </div>
      ...
      <div className="bg-white rounded-xl border border-slate-200 p-4 mb-4">
        <div className="relative">
          ...
          <input
            ...
            className="w-full pl-9 pr-3 py-2 border border-slate-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-purple-500/20"
          />
        </div>
      </div>
                      ...
                      <tr key={item.id} className="hover:bg-slate-50 transition-colors">
                        ...
                        <td className="px-4 py-3">
                          <div className="flex gap-1">
                            <span
                              className={`inline-flex items-center gap-0.5 px-1.5 py-0.5 rounded text-xs font-medium ${item.content_fr ? 'bg-blue-100 text-blue-700' : 'bg-slate-100 text-slate-400'}`}
                            >
                              FR
                            </span>
                            <span
                              className={`inline-flex items-center gap-0.5 px-1.5 py-0.5 rounded text-xs font-medium ${item.content_en ? 'bg-emerald-100 text-emerald-700' : 'bg-slate-100 text-slate-400'}`}
                            >
                              EN
                            </span>
                          </div>
                        ...
                        <td className="px-4 py-3">
                          <button
                            ...
                            className={`inline-flex items-center px-2.5 py-1 rounded-full text-xs font-semibold border ${statusCfg.bg} ${statusCfg.color} ${statusCfg.border} hover:opacity-80 transition-opacity`}
                          >
                            {statusCfg.label}
                          </button>
                        </td>
            ...
            <div className="px-6 py-4 space-y-4">
                ...
                <div>
                  ...
                  <div className="relative">
                    <select
                      ...
                      className="w-full appearance-none px-3 pr-8 py-2 border border-slate-200 rounded-lg text-sm focus:outline-none bg-white"
                    >
              ...
              <div>
                <div className="flex gap-1 bg-slate-100 p-1 rounded-lg mb-3 w-fit">
                  <button
                    onClick={() => setActiveFormTab('fr')}
                    className={`flex items-center gap-1.5 px-3 py-1.5 rounded-md text-xs font-semibold transition-all ${activeFormTab === 'fr' ? 'bg-white text-navy shadow-sm' : 'text-slate-500 hover:text-navy'}`}
                  >
                    <Globe size={11} /> Français
                  </button>
                  <button
                    onClick={() => setActiveFormTab('en')}
                    className={`flex items-center gap-1.5 px-3 py-1.5 rounded-md text-xs font-semibold transition-all ${activeFormTab === 'en' ? 'bg-white text-navy shadow-sm' : 'text-slate-500 hover:text-navy'}`}
                  >
                  ...
                  <div className="space-y-3">
                    ...
                    <div>
                      ...
                      <textarea
                        ...
                        rows={8}
                        className="w-full px-3 py-2 border border-slate-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-purple-500/20 resize-none font-mono"
                ...
                <div className="space-y-3">
                  ...
                  <div className="bg-purple-50 border border-purple-200 rounded-lg p-3">
                    <div className="flex items-center justify-between mb-1">
                      <span className="text-xs font-semibold text-purple-700 bg-purple-100 border border-purple-200 px-2 py-0.5 rounded-full">
                        Version actuelle
                      </span>
                      <span
                        className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs font-semibold ${STATUS_CONFIG[viewingHistoryFor.status]?.bg} ${STATUS_CONFIG[viewingHistoryFor.status]?.color}`}
                      >
                        {STATUS_CONFIG[viewingHistoryFor.status]?.label}
                      </span>
                    </div>
                    <p className="text-sm font-medium text-navy mt-1">
                      {viewingHistoryFor.title_fr}
                    </p>
                    <div className="flex items-center gap-3 mt-1.5">
                      <span className="flex items-center gap-1 text-xs text-slate-500">
                        <Clock size={10} />
                        {formatDateTime(
                          viewingHistoryFor.updated_at || viewingHistoryFor.created_at
                        )}
                      </span>
                    </div>
                  </div>
                  ...
                  {versions.map((v) => (
                    <div key={v.id} className="bg-slate-50 border border-slate-200 rounded-lg p-3">
                      <div className="flex items-center justify-between mb-1">
                        ...
                        <span
                          className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs font-semibold ${STATUS_CONFIG[v.status]?.bg || 'bg-slate-100'} ${STATUS_CONFIG[v.status]?.color || 'text-slate-600'}`}
                        >
                      ...
                      {v.content_fr && (
                        <p className="text-xs text-slate-500 mt-1.5 line-clamp-2 italic">
                          "{v.content_fr.substring(0, 100)}
                          {v.content_fr.length > 100 ? '…' : ''}"
                        </p>
                      )}
                    </div>
                  ))}
          ...
          <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[85vh] overflow-y-auto">
            ...
            <div className="px-6 py-5">
              <div className="flex items-center gap-2 mb-4">
                <span
                  className={`inline-flex items-center px-2.5 py-1 rounded-full text-xs font-semibold border ${STATUS_CONFIG[previewItem.status]?.bg} ${STATUS_CONFIG[previewItem.status]?.color} ${STATUS_CONFIG[previewItem.status]?.border}`}
                >
              ...
              {previewItem.title_en && (
                <p className="text-sm text-slate-500 italic mb-4">{previewItem.title_en}</p>
              )}
              ...
              <div className="mt-4 pt-4 border-t border-slate-100 flex items-center gap-4 text-xs text-slate-400">
                <span className="flex items-center gap-1">
                  <Clock size={10} /> Créé : {formatDate(previewItem.created_at)}
                </span>
                <span className="flex items-center gap-1">
                  <Clock size={10} /> Modifié :{' '}
                  {formatDate(previewItem.updated_at || previewItem.created_at)}
                </span>
              </div>
            </div>
          </div>
