...
const navItems: NavItem[] = [
  { id: 'nav-overview', icon: LayoutDashboard, labelFr: 'Vue d\'ensemble', labelEn: 'Overview', href: '/content-dashboard' },
  { id: 'nav-pages', icon: FileText, labelFr: 'Pages', labelEn: 'Pages', href: '/content-dashboard/pages' },
  { id: 'nav-faq', icon: HelpCircle, labelFr: 'FAQ', labelEn: 'FAQ', href: '/content-dashboard/faq' },
  { id: 'nav-articles', icon: Newspaper, labelFr: 'Articles', labelEn: 'Articles', href: '/content-dashboard/articles' },
  { id: 'nav-glossaire', icon: BookOpen, labelFr: 'Glossaire', labelEn: 'Glossary', href: '/content-dashboard/glossaire' },
];
  ...
  const sidebarContent = (isMobile = false) => (
    <>
      <nav className="flex-1 py-4 px-2 overflow-y-auto scrollbar-thin">
        {(!collapsed || isMobile) && <p className="section-label mb-1.5 px-2">{lang === 'fr' ? 'Contenu' : 'Content'}</p>}
        ...
        <div className={`flex items-center h-16 border-b border-white/5 px-3 flex-shrink-0 ${collapsed ? 'justify-center' : 'gap-3'}`}>
          ...
          {!collapsed && (
            <div className="overflow-hidden">
              ...
              <span className="text-gold text-[9px] font-medium tracking-widest uppercase whitespace-nowrap">{lang === 'fr' ? 'Contenu' : 'Content'}</span>
            </div>
          )}
        </div>
...
export default function ContentLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <RoleGuard allowedRoles={['gestionnaire_contenu', 'admin']}>
      <div className="min-h-screen bg-slate-50 flex">
        <Toaster richColors position="top-right" />
        <ContentSidebar collapsed={collapsed} mobileOpen={mobileOpen} onCloseMobile={() => setMobileOpen(false)} onToggleCollapse={() => setCollapsed(!collapsed)} />
        ...
        <div className={`flex-1 flex flex-col min-w-0 transition-all duration-300 ${collapsed ? 'lg:ml-16' : 'lg:ml-60'}`}>
          <header className="h-16 bg-white border-b border-slate-200 flex items-center px-4 sm:px-6 gap-4 flex-shrink-0 sticky top-0 z-20">
            ...
            <div className="flex items-center gap-2">
              <PenSquare size={16} className="text-purple-600" />
              <span className="font-semibold text-navy text-sm">{lang === 'fr' ? 'Gestion du Contenu' : 'Content Management'}</span>
            </div>
            <div className="ml-auto flex items-center gap-3">
              <span className="hidden sm:inline-flex items-center gap-1.5 bg-purple-50 border border-purple-200 text-purple-700 text-xs font-semibold px-2.5 py-1 rounded-full">
                <PenSquare size={11} /> {lang === 'fr' ? 'Gestionnaire Contenu' : 'Content Manager'}
              </span>
              <button
                onClick={toggleLang}
