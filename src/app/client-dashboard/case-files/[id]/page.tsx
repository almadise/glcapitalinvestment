...
const STATUS_CONFIG: Record<CaseStatus, { labelFr: string; labelEn: string; color: string; bg: string; border: string }> = {
  RECU: { labelFr: 'Reçu', labelEn: 'Received', color: 'text-slate-700', bg: 'bg-slate-100', border: 'border-slate-300' },
  A_COMPLETER: { labelFr: 'À compléter', labelEn: 'To complete', color: 'text-orange-700', bg: 'bg-orange-100', border: 'border-orange-300' },
  EN_ANALYSE: { labelFr: 'En analyse', labelEn: 'Under review', color: 'text-amber-700', bg: 'bg-amber-100', border: 'border-amber-300' },
  EN_REVUE_COMPLIANCE: { labelFr: 'En revue conformité', labelEn: 'Compliance review', color: 'text-blue-700', bg: 'bg-blue-100', border: 'border-blue-300' },
  ELIGIBLE: { labelFr: 'Éligible', labelEn: 'Eligible', color: 'text-emerald-700', bg: 'bg-emerald-100', border: 'border-emerald-300' },
  SOUMIS_PARTENAIRE: { labelFr: 'Soumis partenaire', labelEn: 'Submitted to partner', color: 'text-indigo-700', bg: 'bg-indigo-100', border: 'border-indigo-300' },
  RETOUR_PARTENAIRE: { labelFr: 'Retour partenaire', labelEn: 'Partner feedback', color: 'text-violet-700', bg: 'bg-violet-100', border: 'border-violet-300' },
  EN_NEGOCIATION: { labelFr: 'En négociation', labelEn: 'In negotiation', color: 'text-purple-700', bg: 'bg-purple-100', border: 'border-purple-300' },
  CLOTURE: { labelFr: 'Clôturé', labelEn: 'Closed', color: 'text-teal-700', bg: 'bg-teal-100', border: 'border-teal-300' },
  REJETE: { labelFr: 'Rejeté', labelEn: 'Rejected', color: 'text-red-700', bg: 'bg-red-100', border: 'border-red-300' },
};

function StatusBadge({ status, lang }: { status: CaseStatus; lang: string }) {
  const cfg = STATUS_CONFIG[status] || { labelFr: status, labelEn: status, color: 'text-slate-700', bg: 'bg-slate-100', border: 'border-slate-300' };
  return (
...
function ClientCaseDetailContent() {
      ...
      const enriched: Message[] = await Promise.all(
        (data || []).map(async (msg: Message) => {
          try {
            ...
            return {
              ...msg,
              sender_name: profile?.full_name || profile?.email || t('Équipe GL Capital', 'GL Capital Team'),
            ...
            return {
              ...msg,
              sender_name: msg.sender_id === user?.id ? t('Moi', 'Me') : t('Équipe GL Capital', 'GL Capital Team'),
      ...
      <DashboardLayout>
        <div className="p-6">
          <div className="flex items-center gap-3 bg-red-50 border border-red-200 rounded-xl p-4">
            ...
            <p className="text-red-700 text-sm">{error || t('Dossier introuvable', 'Case file not found')}</p>
          </div>
        </div>
      </DashboardLayout>
    );
  ...
  const tabs = [
    ...
    { key: 'documents' as const, label: `${t('Documents', 'Documents')} (${documents.length})`, icon: Paperclip },
    { key: 'messages' as const, label: `${t('Messages', 'Messages')} (${messages.length})`, icon: MessageSquare },
  ];
  ...
  return (
    <DashboardLayout>
      <div className="p-4 sm:p-6 max-w-4xl mx-auto">
        ...
        <div className="flex gap-1 bg-slate-100 rounded-xl p-1 mb-5">
          {tabs.map((tab) => (
            ...
            >
              ...
              <span className="sm:hidden">{tab.key === 'overview' ? t('Vue', 'View') : tab.key === 'documents' ? t('Docs', 'Docs') : t('Msgs', 'Msgs')}</span>
            </button>
          ))}
        </div>
          ...
          <div className="bg-white border border-slate-200 rounded-2xl p-5 shadow-sm">
            ...
            {documents.length === 0 ? (
              <div className="flex flex-col items-center justify-center py-12 text-center">
                ...
                <p className="text-slate-500 text-sm">{t('Aucun document pour ce dossier', 'No documents for this case file')}</p>
              </div>
            ) : (
                  ...
                  <div key={doc.id} className="flex items-center justify-between gap-3 p-3 bg-slate-50 rounded-xl border border-slate-200">
                    <div className="flex items-center gap-3 min-w-0">
                      ...
                      <div className="min-w-0">
                        ...
                        <p className="text-xs text-slate-400">{formatDate(doc.uploaded_at, lang)}</p>
                      </div>
                    </div>
                    <a
                      href={doc.file_url}
        ...
        {activeTab === 'messages' && (
          <div className="bg-white border border-slate-200 rounded-2xl shadow-sm flex flex-col" style={{ minHeight: '500px' }}>
            {/* Messages header */}
              ...
              {messages.length === 0 ? (
                <div className="flex flex-col items-center justify-center py-12 text-center">
                  ...
                  <p className="text-slate-400 text-xs mt-1">
                    {t('Envoyez un message à l\'équipe GL Capital', 'Send a message to the GL Capital team')}
                  </p>
                </div>
              ) : (
                messages.map((msg) => {
                  ...
                  return (
                    <div key={msg.id} className={`flex gap-3 ${isOwn ? 'flex-row-reverse' : 'flex-row'}`}>
                      {/* Avatar */}
                      <div className={`flex-shrink-0 w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold ${msg.is_staff ? 'bg-navy text-white' : 'bg-gold text-navy'}`}>
                      ...
                      <div className={`max-w-xs sm:max-w-sm lg:max-w-md ${isOwn ? 'items-end' : 'items-start'} flex flex-col gap-1`}>
                        <div className={`flex items-center gap-2 text-xs text-slate-400 ${isOwn ? 'flex-row-reverse' : 'flex-row'}`}>
                          <span className="font-medium text-slate-600">
                            {isOwn ? t('Moi', 'Me') : (msg.sender_name || t('Équipe GL Capital', 'GL Capital Team'))}
                          </span>
                          {msg.is_staff && !isOwn && (
                        ...
                        <div className={`px-4 py-3 rounded-2xl text-sm leading-relaxed ${
                          isOwn
                            ? 'bg-navy text-white rounded-tr-sm' :'bg-slate-100 text-slate-800 rounded-tl-sm'
                        }`}>
                          {msg.body}
                        </div>
                        <div className={`flex items-center gap-1 text-xs text-slate-400 ${isOwn ? 'flex-row-reverse' : 'flex-row'}`}>
                          <Calendar size={10} />
            ...
            <div className="border-t border-slate-100 p-4">
              ...
              <div className="flex gap-3">
                <textarea
                  ...
                  placeholder={t('Écrivez un message... (Entrée pour envoyer)', 'Write a message... (Enter to send)')}
