      ...
      <div className="hidden lg:flex lg:w-[52%] xl:w-[55%] bg-gradient-navy flex-col justify-between p-12 xl:p-16 relative overflow-hidden">
        ...
        <div className="relative z-10">
          <div className="inline-flex items-center gap-2 bg-gold/10 border border-gold/20 rounded-full px-4 py-1.5 mb-6">
            ...
            <span className="text-gold text-xs font-semibold tracking-widest uppercase">Portail sécurisé</span>
          </div>
          ...
          <div className="space-y-3">
            {[
              { icon: Shield, label: 'Chiffrement de bout en bout', sub: 'Documents protégés par URLs signées' },
              { icon: Lock, label: 'Authentification sécurisée', sub: 'Supabase Auth · MFA disponible' },
        ...
        <div className="relative z-10 text-xs text-slate-600 border-t border-white/10 pt-6">
          GL Capital n&apos;est pas une banque. Aucun dépôt n&apos;est accepté. Aucun rendement n&apos;est garanti.
        </div>
      </div>
      ...
      <div className="flex-1 flex flex-col justify-center px-4 py-8 sm:px-8 sm:py-10 lg:px-14 xl:px-20 overflow-y-auto">
        ...
        <div className="w-full max-w-md mx-auto">
            ...
            <form onSubmit={handleLogin} className="space-y-5">
              ...
              <div>
                <div className="flex items-center justify-between mb-2">
                  ...
                  <Link href="/forgot-password" className="text-xs text-gold hover:text-gold-light transition-colors font-medium">
                    Mot de passe oublié ?
                  </Link>
                </div>
                <div className="relative">
            ...
            <form onSubmit={registerForm.handleSubmit(handleRegister)} className="space-y-4">
              <div>
                ...
                {registerForm.formState.errors.fullName && (
                  <p className="text-red-500 text-xs mt-1.5">{registerForm.formState.errors.fullName.message}</p>
                )}
              </div>
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    className="input-field pl-10"
                    {...registerForm.register('organization', { required: "L'organisation est requise." })}
                  />
                </div>
                {registerForm.formState.errors.organization && (
                  <p className="text-red-500 text-xs mt-1.5">{registerForm.formState.errors.organization.message}</p>
                )}
              </div>
              ...
              <div>
                ...
                {registerForm.formState.errors.email && (
                  <p className="text-red-500 text-xs mt-1.5">{registerForm.formState.errors.email.message}</p>
                )}
              </div>
              ...
              <div className="flex items-start gap-2.5">
                ...
                <label htmlFor="terms" className="text-xs text-slate-600 leading-relaxed cursor-pointer">
                  J&apos;accepte les{' '}
                  <a href="#" className="text-navy underline hover:text-gold">conditions d&apos;utilisation</a>
                  {' '}et la{' '}
                  <a href="#" className="text-navy underline hover:text-gold">politique de confidentialité</a>.
                  Je confirme que les informations fournies sont exactes et traçables.
                </label>
              </div>
              {registerForm.formState.errors.acceptTerms && (
                <p className="text-red-500 text-xs">{registerForm.formState.errors.acceptTerms.message}</p>
              )}

              {/* Notification preferences */}
              <div className="bg-slate-50 border border-slate-200 rounded-xl p-4">
                <p className="text-xs font-semibold text-slate-700 mb-3">
                  {lang === 'fr' ? 'Préférences de notifications email' : 'Email notification preferences'}
                </p>
                <div className="space-y-2.5">
                  {[
                    { name: 'notifDossierUpdates' as const, label: lang === 'fr' ? 'Mises à jour de mes dossiers' : 'My file updates' },
                    { name: 'notifPipelineAlerts' as const, label: lang === 'fr' ? 'Alertes pipeline & statuts' : 'Pipeline & status alerts' },
                    { name: 'notifDocumentsPending' as const, label: lang === 'fr' ? 'Documents en attente de dépôt' : 'Documents pending upload' },
                  ].map(({ name, label }) => (
                    <div key={name} className="flex items-center gap-2.5">
                      ...
                      />
                      <label htmlFor={name} className="text-xs text-slate-600 cursor-pointer">{label}</label>
                    </div>
                  ))}
                </div>
              </div>
          ...
          <div className="mt-6 text-center">
            <Link href="/home-page" className="text-xs text-slate-500 hover:text-navy transition-colors">
