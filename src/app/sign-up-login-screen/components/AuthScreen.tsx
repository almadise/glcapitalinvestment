...
export default function AuthScreen() {
  ...
  const handleRegister = registerForm.handleSubmit(async (data) => {
    if (data.password !== data.confirmPassword) {
      registerForm.setError('confirmPassword', { message: t('Les mots de passe ne correspondent pas', 'Passwords do not match') });
      return;
    }
    setIsLoading(true);
    try {
    ...
    <div className="min-h-screen flex bg-navy-950">
      ...
      <div className="hidden lg:flex flex-col justify-between w-1/2 bg-gradient-hero p-12 relative overflow-hidden">
        ...
        <div className="relative z-10">
          <div className="flex items-center gap-3 mb-12">
            ...
            <div>
              ...
              <p className="text-gold-500 text-xs font-mono tracking-widest uppercase">Investment SA</p>
            </div>
          </div>

          <h1 className="text-4xl font-bold text-white mb-4 leading-tight">
            {t('Portail de Financement', 'Secure Financing')}<br />
        ...
        <div className="relative z-10 space-y-4">
          {[
            { icon: Shield, textFr: 'Traitement conforme KYC/AML', textEn: 'KYC/AML compliant processing', subFr: 'Chaque dossier est contrôlé', subEn: 'Every dossier screened' },
            { icon: Lock, textFr: 'Stockage chiffré TLS', textEn: 'TLS encrypted document storage', subFr: 'Sécurité de bout en bout', subEn: 'End-to-end security' },
            { icon: CheckCircle2, textFr: 'Piste d\'audit immuable', textEn: 'Immutable audit trail', subFr: 'Traçabilité complète', subEn: 'Full traceability' },
            { icon: Smartphone, textFr: 'Authentification multi-facteurs', textEn: 'Multi-factor authentication', subFr: 'MFA disponible', subEn: 'MFA available' },
          ].map((item, i) => (
      ...
      <div className="flex-1 flex flex-col justify-center px-6 lg:px-16 py-12 overflow-y-auto">
        ...
        <div className="max-w-md w-full mx-auto">
          ...
          {verificationSent ? (
            <div className="animate-scale-in">
              <div className="text-center mb-8">
                ...
                <h2 className="text-white text-2xl font-bold mb-2">{t('Vérifiez votre email', 'Verify your email')}</h2>
                <p className="text-white/50 text-sm leading-relaxed">
                  {t('Nous avons envoyé un lien de vérification à', 'We sent a verification link to')}<br />
              ...
              <div className="bg-navy-800 border border-navy-600 rounded-2xl p-5 mb-6 space-y-3">
                <div className="flex items-start gap-3">
                  ...
                  <p className="text-white/70 text-xs">{t('Cliquez sur le lien dans l\'email pour activer votre compte', 'Click the link in the email to activate your account')}</p>
                </div>
                <div className="flex items-start gap-3">
                  <CheckCircle2 size={14} className="text-emerald-400 flex-shrink-0 mt-0.5" />
                  <p className="text-white/70 text-xs">{t('Le lien expire dans 24 heures', 'The link expires in 24 hours')}</p>
                </div>
                <div className="flex items-start gap-3">
                  <CheckCircle2 size={14} className="text-emerald-400 flex-shrink-0 mt-0.5" />
                  <p className="text-white/70 text-xs">{t('Vérifiez votre dossier spam si vous ne voyez pas l\'email', 'Check your spam folder if you do not see the email')}</p>
                </div>
              </div>
            ...
            <>
              ...
              {!mfaStep && (
                <div className="flex bg-navy-800 rounded-xl p-1 mb-8">
                  {(['login', 'register'] as AuthMode[]).map((m) => (
                    ...
                    >
                      {m === 'login' ? t('Connexion', 'Sign In') : t('Créer un compte', 'Create Account')}
                    </button>
                  ))}
                </div>
              )}
              ...
              {mfaStep ? (
                ...
                <form onSubmit={handleLogin} className="space-y-5 animate-fade-in">
                  <div>
                    <h2 className="text-white text-2xl font-bold mb-1">{t('Bon retour', 'Welcome back')}</h2>
                    <p className="text-white/40 text-sm">{t('Connectez-vous à votre portail GL Capital', 'Sign in to your GL Capital portal')}</p>
                  </div>
                  ...
                  <div>
                    ...
                    <div className="relative">
                      ...
                      <input
                        type="email"
                        {...loginForm.register('email', { required: t('Email requis', 'Email is required') })}
                  ...
                  <div>
                    ...
                    <div className="relative">
                      ...
                      <input
                        ...
                        {...loginForm.register('password', { required: t('Mot de passe requis', 'Password is required') })}
                        placeholder="••••••••••"
                  ...
                  <div className="flex items-center justify-between">
                    <label className="flex items-center gap-2 cursor-pointer">
                      ...
                      />
                      <span className="text-white/50 text-sm">{t('Se souvenir de moi', 'Remember me')}</span>
                    </label>
                    <button type="button" className="text-gold-400 text-sm hover:text-gold-300 transition-colors">
                ...
                <form onSubmit={handleRegister} className="space-y-4 animate-fade-in">
                  <div>
                    <h2 className="text-white text-2xl font-bold mb-1">{t('Créer votre compte', 'Create your account')}</h2>
                    <p className="text-white/40 text-sm">{t('Accédez au portail sécurisé GL Capital', 'Access the GL Capital secure portal')}</p>
                  </div>

                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      <label className="block text-white/70 text-xs font-medium mb-1.5">{t('Prénom', 'First Name')}</label>
                      <input
                        {...registerForm.register('firstName', { required: t('Requis', 'Required') })}
                        placeholder="Amadou"
                    ...
                    <div>
                      <label className="block text-white/70 text-xs font-medium mb-1.5">{t('Nom', 'Last Name')}</label>
                      <input
                        {...registerForm.register('lastName', { required: t('Requis', 'Required') })}
                        placeholder="Diallo"
                        className="w-full bg-navy-800 border border-navy-600 text-white px-3 py-2.5 rounded-xl focus:outline-none focus:border-gold-500 transition-colors text-sm placeholder:text-white/20"
                      />
                    </div>
                  ...
                  <div>
                    <label className="block text-white/70 text-xs font-medium mb-1.5">{t('Email professionnel', 'Professional Email')}</label>
                    <div className="relative">
                      <Mail size={14} className="absolute left-3 top-1/2 -translate-y-1/2 text-white/30" />
                      <input
                        type="email"
                        {...registerForm.register('email', { required: t('Email requis', 'Email is required') })}
                        placeholder="vous@votreentreprise.com"
                    ...
                    <p className="text-white/30 text-xs mt-1">{t('Utilisez votre email professionnel. Gmail/Hotmail déconseillé.', 'Use your corporate email. Gmail/Hotmail not recommended.')}</p>
                  </div>

                  <div>
                    <label className="block text-white/70 text-xs font-medium mb-1.5">{t('Organisation / Entreprise', 'Organization / Company')}</label>
                    <div className="relative">
                      <Building2 size={14} className="absolute left-3 top-1/2 -translate-y-1/2 text-white/30" />
                      <input
                        {...registerForm.register('organization', { required: t('Organisation requise', 'Organization is required') })}
                  ...
                  <div>
                    <label className="block text-white/70 text-xs font-medium mb-1.5">{t('Pays d\'immatriculation', 'Country of Registration')}</label>
                    <select
                      {...registerForm.register('country', { required: t('Pays requis', 'Country is required') })}
                  ...
                  <div>
                    <label className="block text-white/70 text-xs font-medium mb-1.5">{t('Mot de passe', 'Password')}</label>
                    <div className="relative">
                      ...
                      <input
                        ...
                        {...registerForm.register('password', {
                          ...
                          minLength: { value: 10, message: t('Minimum 10 caractères', 'Minimum 10 characters') },
                          pattern: { value: /(?=.*[A-Z])(?=.*[0-9])(?=.*[!@#$%^&*])/, message: t('Doit inclure majuscule, chiffre et caractère spécial', 'Must include uppercase, number, and special character') },
                        })}
                        placeholder={t('Min. 10 car., majuscule, chiffre, symbole', 'Min. 10 chars, uppercase, number, symbol')}
                  ...
                  <div>
                    <label className="block text-white/70 text-xs font-medium mb-1.5">{t('Confirmer le mot de passe', 'Confirm Password')}</label>
                    <div className="relative">
                      <Lock size={14} className="absolute left-3 top-1/2 -translate-y-1/2 text-white/30" />
                      <input
                        type={showConfirmPassword ? 'text' : 'password'}
                        {...registerForm.register('confirmPassword', { required: t('Veuillez confirmer votre mot de passe', 'Please confirm your password') })}
                  ...
                  <div className="p-3 bg-gold-500/5 border border-gold-500/20 rounded-xl space-y-2">
                    <label className="flex items-start gap-2 cursor-pointer">
                      <input type="checkbox" {...registerForm.register('acceptTerms', { required: true })} className="mt-0.5 accent-gold-500" />
                      <span className="text-white/60 text-xs leading-relaxed">
                        {t('J\'accepte les ', 'I accept the ')}
                        <span className="text-gold-400 underline cursor-pointer">{t('Conditions d\'utilisation', 'Terms of Use')}</span>
                        {t(' et la ', ' and ')}
                        <span className="text-gold-400 underline cursor-pointer">{t('Politique de confidentialité', 'Privacy Policy')}</span>
                      </span>
                    </label>
                    <label className="flex items-start gap-2 cursor-pointer">
                      <input type="checkbox" {...registerForm.register('acceptNcnda', { required: true })} className="mt-0.5 accent-gold-500" />
                      <span className="text-white/60 text-xs leading-relaxed">
                  ...
                  >
                    {isLoading ? (
                      ...
                      <>{t('Créer un compte sécurisé', 'Create Secure Account')} <ArrowRight size={16} /></>
                    )}
                  </button>
                </form>
              )}
          ...
          <div className="mt-6 text-center">
            <Link href="/home-page" className="text-white/30 text-xs hover:text-white/50 transition-colors">
