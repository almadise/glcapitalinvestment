...
export default function BackOfficeDashboard() {
  ...
  const [detailTab, setDetailTab] = useState<'overview' | 'compliance' | 'documents' | 'notes' | 'timeline' | 'audit'>('overview');
  const [internalNote, setInternalNote] = useState('');
  const [activeMainTab, setActiveMainTab] = useState<'dossiers' | 'audit' | 'partners' | 'notifications'>('dossiers');
  const [statusDropdownOpen, setStatusDropdownOpen] = useState<string | null>(null);
  const [statusChangeNote, setStatusChangeNote] = useState('');
  const [pendingStatusChange, setPendingStatusChange] = useState<{ caseId: string; newStatus: DossierStatus } | null>(null);
  ...
  const [kpi, setKpi] = useState<KPIData>({
    totalActive: 0, complianceQueue: 0, riskAlerts: 0,
    eligible: 0, totalCases: 0, rejectedThisMonth: 0,
  });
    ...
    try {
      ...
      if (error) {
        ...
        setKpi({
          ...
          riskAlerts: mapped.filter((c) => c.riskTags?.some((tag) => ['risk', 'pep', 'sanction'].some((k) => tag.toLowerCase().includes(k)))).length,
    ...
    try {
      ...
      if (user && selectedCase) {
        await supabase.from('audit_logs').insert({
          ...
          target: selectedCase.ref,
          metadata: { actor_email: user.email, detail: `Admin téléchargé : ${doc.file_name}`, severity: 'info' },
        });
      }
    ...
    <div className="max-w-screen-2xl mx-auto">
      ...
      {pendingStatusChange && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          <div className="absolute inset-0 bg-black/40" onClick={() => setPendingStatusChange(null)} />
          <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-md p-6">
            ...
            <div className="flex gap-3">
              <button onClick={() => setPendingStatusChange(null)} className="flex-1 py-2 border border-gray-200 rounded-xl text-sm text-gray-600 hover:bg-gray-50 transition-colors">Annuler</button>
              <button
                onClick={() => applyStatusChange(pendingStatusChange.caseId, pendingStatusChange.newStatus, statusChangeNote)}
          ...
          <p className="text-gray-500 text-sm mt-1 flex items-center gap-2">
            ...
            {kpi.riskAlerts > 0 && (
              <>
                ...
                <span className="text-xs text-red-500 font-medium">{kpi.riskAlerts} {t('alerte(s) risque', 'risk alert(s)')}</span>
              </>
            )}
          </p>
        </div>
        <div className="flex items-center gap-3">
          <button onClick={() => toast.info(t('Export audit log...', 'Exporting audit log...'))} className="flex items-center gap-2 px-4 py-2 border border-gray-200 text-gray-600 text-sm font-medium rounded-xl hover:bg-gray-50 transition-colors">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-6 gap-4 mb-8">
        {kpiCards.map((card) => (
          <div key={card.id} className={`border rounded-2xl p-5 shadow-card ${card.color}`}>
            ...
            <div className={`text-2xl font-bold tabular-nums mb-1 ${card.valueColor}`}>{card.value}</div>
      ...
      {activeMainTab === 'dossiers' && (
        <div className={`flex gap-6 ${selectedCase ? '' : ''}`}>
          <div className={`${selectedCase ? 'flex-1 min-w-0' : 'w-full'} bg-white border border-gray-200 rounded-2xl overflow-hidden shadow-card`}>
            <div className="px-6 py-4 border-b border-gray-100 flex items-center gap-3 flex-wrap">
              <div className="relative flex-1 min-w-48">
                <Search size={14} className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400" />
                <input type="text" placeholder={t('Rechercher réf, organisation, pays...', 'Search ref, organisation, country...')} value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)}
              ...
              <div className="flex items-center gap-2">
                ...
                <select value={statusFilter} onChange={(e) => setStatusFilter(e.target.value)} className="px-3 py-2 border border-gray-200 rounded-xl text-sm focus:outline-none focus:border-navy-400 transition-colors bg-white">
                  <option value="ALL">{t('Tous les statuts', 'All statuses')}</option>
                  {statusOptions.map((s) => <option key={`filter-${s}`} value={s}>{STATUS_LABELS[s] || s}</option>)}
                </select>
              </div>
              <span className="text-xs text-gray-400 font-mono">{filteredCases.length} {t('dossier(s)', 'file(s)')}</span>
            </div>
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="bg-gray-50 border-b border-gray-100">
                      <th className="px-4 py-3 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">Référence</th>
                      ...
                      <th className="px-4 py-3 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">Score KYC</th>
                      <th className="px-4 py-3 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">Analyste</th>
                      <th className="px-4 py-3 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">Mis à jour</th>
                  ...
                  <tbody className="divide-y divide-gray-50">
                    {filteredCases.map((c) => (
                      <tr key={c.id} className={`hover:bg-gray-50 transition-colors cursor-pointer ${selectedCase?.id === c.id ? 'bg-gold-50 border-l-2 border-l-gold-500' : ''}`}
                        ...
                        <td className="px-4 py-3 text-xs font-semibold text-navy-900 tabular-nums">{c.amount}</td>
                        <td className="px-4 py-3" onClick={(e) => e.stopPropagation()}>
                          <div className="relative">
                            <button onClick={() => setStatusDropdownOpen(statusDropdownOpen === c.id ? null : c.id)} className="flex items-center gap-1 group">
                              <StatusBadge status={c.status} size="sm" />
                              <ChevronDown size={10} className="text-gray-400 group-hover:text-gray-600 transition-colors" />
                            </button>
                            {statusDropdownOpen === c.id && (
                              <div className="absolute top-full left-0 mt-1 w-56 bg-white border border-gray-200 rounded-xl shadow-lg z-20 overflow-hidden">
                                <div className="p-1 max-h-64 overflow-y-auto">
                                  {statusOptions.map((s) => {
                                    const isGated = COMPLIANCE_GATED_STATUSES.includes(s) && !isComplianceOrAdmin;
                                    return (
                                      <button key={`status-opt-${s}`} onClick={() => !isGated && handleStatusChange(c.id, s)}
                                        disabled={isGated}
                        ...
                        <td className="px-4 py-3">
                          <div className="flex items-center gap-2">
                            <div className="w-12 h-1.5 bg-gray-100 rounded-full overflow-hidden">
                              <div className={`h-full rounded-full ${c.complianceScore >= 80 ? 'bg-emerald-500' : c.complianceScore >= 60 ? 'bg-gold-500' : c.complianceScore >= 40 ? 'bg-amber-500' : 'bg-red-500'}`} style={{ width: `${c.complianceScore}%` }} />
                            </div>
                            <span className={`text-[10px] font-mono font-semibold ${c.complianceScore >= 80 ? 'text-emerald-600' : c.complianceScore >= 60 ? 'text-gold-600' : c.complianceScore >= 40 ? 'text-amber-600' : 'text-red-600'}`}>{c.complianceScore}%</span>
                          </div>
                        </td>
                        <td className="px-4 py-3">
                          <div className="flex items-center gap-1.5">
                            <div className="w-5 h-5 rounded-full bg-navy-200 flex items-center justify-center flex-shrink-0">
                              <span className="text-[8px] font-bold text-navy-700">{c.analyst !== 'Non assigné' ? c.analyst.split(' ').map((n) => n[0]).join('') : '?'}</span>
                            </div>
                            <span className="text-xs text-gray-600 truncate max-w-[80px]">{c.analyst.split(' ')[0]}</span>
                          </div>
                        </td>
                        <td className="px-4 py-3 text-xs text-gray-500 whitespace-nowrap">{c.lastUpdate}</td>
                        <td className="px-4 py-3" onClick={(e) => e.stopPropagation()}>
                          <div className="flex items-center gap-1">
                            <button onClick={() => { handleSelectCase(c); setDetailTab('overview'); }} className="p-1.5 text-gray-400 hover:text-navy-700 hover:bg-gray-100 rounded-lg transition-colors" title="Voir le dossier"><Eye size={13} /></button>
                            <button onClick={() => { handleSelectCase(c); setDetailTab('timeline'); }} className="p-1.5 text-gray-400 hover:text-navy-700 hover:bg-gray-100 rounded-lg transition-colors" title="Timeline"><History size={13} /></button>
                            <button onClick={() => toast.info(`Messages pour ${c.ref}`)} className="p-1.5 text-gray-400 hover:text-navy-700 hover:bg-gray-100 rounded-lg transition-colors" title="Messages"><MessageSquare size={13} /></button>
                          </div>
                        </td>
                      </tr>
                    ))}
            ...
            <div className="px-6 py-4 border-t border-gray-100 bg-gray-50">
              <p className="text-xs text-gray-400">{t('Affichage de', 'Showing')} <span className="font-semibold text-gray-600">{filteredCases.length}</span> {t('sur', 'of')} <span className="font-semibold text-gray-600">{cases.length}</span> {t('dossiers', 'files')}</p>
            </div>
          </div>
          ...
          {selectedCase && (
            <div className="w-96 flex-shrink-0 bg-white border border-gray-200 rounded-2xl overflow-hidden shadow-card">
              <div className="px-5 py-4 border-b border-gray-100 flex items-start justify-between bg-navy-900">
                ...
                <button onClick={() => handleSelectCase(null)} className="p-1.5 text-white/40 hover:text-white transition-colors rounded-lg hover:bg-white/10"><X size={16} /></button>
              </div>

              <div className="flex border-b border-gray-100 bg-gray-50 overflow-x-auto">
                {(['overview', 'compliance', 'documents', 'notes', 'timeline', 'audit'] as const).map((tab) => (
                  <button key={`detail-tab-${tab}`} onClick={() => setDetailTab(tab)}
                    ...
                    {tab === 'timeline' ? <span className="flex items-center gap-1"><History size={10} />Timeline</span> : tab}
                    {tab === 'documents' && dossierDocuments.length > 0 && (
                      <span className="ml-1 text-[9px] font-bold bg-navy-900 text-white px-1 py-0.5 rounded-full">{dossierDocuments.length}</span>
                    )}
                  </button>
                ))}
              </div>
              ...
              <div className="overflow-y-auto max-h-[calc(100vh-400px)]">
                  ...
                  <div className="p-5 space-y-4">
                    <dl className="space-y-3">
                      {[
                        ...
                        <div key={`detail-${item.label}`} className="flex items-center justify-between py-1 border-b border-gray-50">
                          <dt className="text-xs text-gray-500">{item.label}</dt>
                          <dd className={`text-xs font-semibold text-navy-800 ${item.mono ? 'font-mono' : ''}`}>{item.value}</dd>
                        </div>
                      ))}
                    </dl>
                    ...
                    <div>
                      <p className="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-2">Tags risque</p>
                      <div className="flex flex-wrap gap-1.5">
                        {(selectedCase.riskTags || []).length === 0 ? (
                          ...
                          <span key={`risk-${tag}`} className={`text-[10px] font-medium px-2 py-0.5 rounded-full border ${tag.toLowerCase().includes('risk') || tag.toLowerCase().includes('pep') || tag.toLowerCase().includes('sanction') ? 'bg-red-50 text-red-600 border-red-200' : 'bg-gray-100 text-gray-600 border-gray-200'}`}>{tag}</span>
                        ))}
                      </div>
                    </div>
                    <div className="space-y-2 pt-2">
                      <p className="text-xs font-semibold text-gray-500 uppercase tracking-wider">Actions rapides</p>
                      <button onClick={() => { setDetailTab('documents'); }} className="w-full flex items-center gap-2 px-3 py-2.5 text-xs font-medium text-navy-700 border border-gray-200 rounded-xl hover:bg-gray-50 transition-colors"><FileText size={13} />Voir les documents ({dossierDocuments.length})</button>
                      {isComplianceOrAdmin && (
                        <button onClick={() => handleStatusChange(selectedCase.id, 'SOUMIS_PARTENAIRE')} className="w-full flex items-center gap-2 px-3 py-2.5 text-xs font-medium text-gold-700 border border-gold-200 bg-gold-50 rounded-xl hover:bg-gold-100 transition-colors"><Network size={13} />Soumettre à institution agréée</button>
                      )}
                    </div>
                  </div>
                )}
                        ...
                        <div className="space-y-4">
                          {statusHistory.map((entry, idx) => (
                            <div key={entry.id} className="relative pl-8">
                              <div className={`absolute left-0 w-6 h-6 rounded-full border-2 flex items-center justify-center ${idx === 0 ? 'bg-navy-900 border-navy-900' : 'bg-white border-gray-300'}`}>
                                {idx === 0 ? <div className="w-2 h-2 border border-white rounded-full" /> : <div className="w-1.5 h-1.5 rounded-full bg-gray-400" />}
                              </div>
                              <div className={`p-3 rounded-xl border ${idx === 0 ? 'bg-navy-50 border-navy-200' : 'bg-gray-50 border-gray-200'}`}>
                                <div className="flex items-center gap-2 mb-1 flex-wrap">
                                  {entry.old_status && (
                                    <>
                                      <StatusBadge status={entry.old_status as DossierStatus} size="sm" />
                                      <ChevronRight size={10} className="text-gray-400" />
                                    </>
                                  )}
                                  <StatusBadge status={entry.new_status as DossierStatus} size="sm" />
                                </div>
                ...
                {detailTab === 'compliance' && (
                  <div className="p-5 space-y-4">
                    <div className="flex items-center justify-between mb-2">
                      <p className="text-xs font-bold text-gray-700 uppercase tracking-wider">Checklist KYC/AML</p>
                      <span className={`text-xs font-bold font-mono ${selectedCase.complianceScore >= 80 ? 'text-emerald-600' : selectedCase.complianceScore >= 60 ? 'text-gold-600' : selectedCase.complianceScore >= 40 ? 'text-amber-600' : 'text-red-600'}`}>{selectedCase.complianceScore}%</span>
                    </div>
                    <div className="space-y-2">
                      {[
                        { id: 'kyc-1', label: 'Vérification identité', status: selectedCase.complianceScore > 20 ? 'pass' : 'pending', detail: 'Registre + pièces dirigeants' },
                        { id: 'kyc-2', label: 'Identification UBO', status: selectedCase.complianceScore > 40 ? 'pass' : 'pending', detail: 'Seuil 25%+ de détention' },
                        { id: 'kyc-3', label: 'Origine des fonds', status: selectedCase.complianceScore > 55 ? 'pass' : 'fail', detail: 'Relevés bancaires ou rapports d\'audit' },
                        { id: 'kyc-4', label: 'Screening sanctions', status: selectedCase.complianceScore > 60 ? 'pass' : 'pending', detail: 'OFAC, UE, ONU, HMT' },
                        { id: 'kyc-5', label: 'Vérification PPE', status: selectedCase.complianceScore > 70 ? 'pass' : (selectedCase.riskTags || []).includes('PEP Flag') ? 'fail' : 'pending', detail: 'Personnes politiquement exposées' },
                        { id: 'kyc-6', label: 'Risque pays', status: selectedCase.complianceScore > 75 ? 'pass' : 'pending', detail: 'FATF, Basel AML Index' },
                        { id: 'kyc-7', label: 'Traçabilité des fonds', status: selectedCase.complianceScore > 85 ? 'pass' : 'pending', detail: 'Origine bout en bout' },
                      ].map((check) => (
                        <div key={check.id} className={`flex items-start gap-3 p-3 rounded-xl border ${check.status === 'pass' ? 'bg-emerald-50 border-emerald-200' : check.status === 'fail' ? 'bg-red-50 border-red-200' : 'bg-gray-50 border-gray-200'}`}>
                          {check.status === 'pass' ? <CheckCircle2 size={14} className="text-emerald-500 flex-shrink-0 mt-0.5" /> : check.status === 'fail' ? <AlertTriangle size={14} className="text-red-500 flex-shrink-0 mt-0.5" /> : <Clock size={14} className="text-gray-400 flex-shrink-0 mt-0.5" />}
                          <div>
                            <p className={`text-xs font-semibold ${check.status === 'pass' ? 'text-emerald-700' : check.status === 'fail' ? 'text-red-700' : 'text-gray-600'}`}>{check.label}</p>
                            <p className="text-[10px] text-gray-400">{check.detail}</p>
                          </div>
                          <div className="ml-auto flex-shrink-0"><button onClick={() => toast.info(`Mise à jour : ${check.label}`)} className="p-1 text-gray-400 hover:text-gray-600 transition-colors"><Edit3 size={11} /></button></div>
                        </div>
                      ))}
                    </div>
                    {isComplianceOrAdmin && (
                ...
                {detailTab === 'documents' && (
                  <div className="p-5 space-y-3">
                    <div className="flex items-center justify-between mb-2">
                      <p className="text-xs font-bold text-gray-700 uppercase tracking-wider">Documents soumis</p>
                      <span className="text-[10px] text-gray-400 font-mono">{dossierDocuments.length} fichier(s)</span>
                    </div>
                    <div className="p-2.5 bg-navy-900/5 border border-navy-900/10 rounded-xl flex items-center gap-2">
                      ...
                      <p className="text-navy-700 text-[10px]">URLs signées 2 minutes. Tous les accès sont journalisés.</p>
                    </div>
                    {dossierDocuments.length === 0 ? (
                      ...
                      <div className="space-y-2">
                        {dossierDocuments.map((doc) => (
                          <div key={doc.id} className={`p-3 rounded-xl border ${doc.scan_passed ? 'bg-white border-gray-200' : 'bg-amber-50 border-amber-200'}`}>
                            <div className="flex items-start gap-2.5">
                              ...
                              <div className="flex-1 min-w-0">
                                <p className="text-xs font-semibold text-navy-800 truncate">{doc.file_name}</p>
                                <div className="flex items-center gap-1.5 mt-0.5 flex-wrap">
                                  <span className="text-[9px] text-gray-400">{doc.doc_type}</span>
                                  <span className="text-gray-300">·</span>
                                  <span className="text-[9px] text-gray-400">{formatFileSize(doc.file_size)}</span>
                                  <span className="text-gray-300">·</span>
                                  {doc.scan_passed ? (
                                    <span className="text-[9px] text-emerald-600 font-semibold flex items-center gap-0.5"><CheckCircle2 size={9} /> Vérifié</span>
                                  ) : (
                                    <span className="text-[9px] text-amber-600 font-semibold">En attente scan</span>
                                  )}
                                </div>
                              </div>
                              <button onClick={() => handleDocumentDownload(doc)} disabled={downloadingDocId === doc.id || !doc.scan_passed}
                                ...
                                {downloadingDocId === doc.id ? <div className="w-3 h-3 border border-navy-700/30 border-t-navy-700 rounded-full animate-spin" /> : <Download size={10} />}
                                DL
                              </button>
                            </div>
                          </div>
                  ...
                  <div className="p-5 space-y-4">
                    ...
                    <div className="space-y-3">
                      {auditLogs.filter((l) => l.target_ref === selectedCase.ref && l.action === 'NOTE_ADDED').slice(0, 5).map((log) => (
                        <div key={log.id} className="p-3 bg-gray-50 rounded-xl border border-gray-200">
                          <div className="flex items-center justify-between mb-2">
                            <span className="text-xs font-semibold text-navy-800">{log.actor_email}</span>
                            <span className="text-[10px] text-gray-400">{new Date(log.created_at).toLocaleDateString('fr-FR', { day: '2-digit', month: 'short', year: 'numeric', hour: '2-digit', minute: '2-digit' })}</span>
                          </div>
                          <p className="text-xs text-gray-600 leading-relaxed">{log.detail}</p>
                        </div>
                      ))}
                      {auditLogs.filter((l) => l.target_ref === selectedCase.ref && l.action === 'NOTE_ADDED').length === 0 && (
                        <p className="text-xs text-gray-400 text-center py-4">Aucune note pour ce dossier</p>
                      )}
                    </div>
                    <div>
                      ...
                      {auditLogs.filter((log) => log.target_ref === selectedCase.ref).map((log) => (
                        <div key={log.id} className={`p-3 rounded-xl border text-xs ${log.severity === 'critical' ? 'bg-red-50 border-red-200' : log.severity === 'sensitive' ? 'bg-amber-50 border-amber-200' : 'bg-gray-50 border-gray-200'}`}>
                          <div className="flex items-center gap-3 mb-1">
                            <span className="font-mono font-semibold text-navy-800 text-[10px]">{log.action}</span>
                            <span className="text-[10px] text-gray-400">{new Date(log.created_at).toLocaleDateString('fr-FR', { day: '2-digit', month: 'short', year: 'numeric' })}</span>
                          </div>
                          <p className="text-gray-600 mb-1">{log.detail}</p>
                          <div className="flex items-center gap-3 text-[10px] text-gray-400 font-mono">
                            <span>{log.actor_email}</span>
                            {log.ip_address && <><span>·</span><span>IP: {log.ip_address}</span></>}
                          </div>
                        </div>
                      ))}
                      {auditLogs.filter((log) => log.target_ref === selectedCase.ref).length === 0 && (
                        <p className="text-xs text-gray-400 text-center py-4">Aucune entrée d'audit pour ce dossier</p>
                      )}
                    </div>
                  </div>
                )}
        ...
        <div className="bg-white border border-gray-200 rounded-2xl overflow-hidden shadow-card">
          <div className="px-6 py-4 border-b border-gray-100 flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <span className="text-[10px] font-mono text-gray-400 bg-gray-100 px-2 py-0.5 rounded-full">Append-only · Inaltérable</span>
            </div>
            <button onClick={() => toast.info('Export CSV du journal audit...')} className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium text-gray-600 border border-gray-200 rounded-lg hover:bg-gray-50 transition-colors">
              <Download size={12} />Export CSV
            </button>
          </div>
          {auditLogs.length === 0 ? (
            <div className="p-12 text-center"><p className="text-gray-400 text-sm">Aucune entrée d'audit</p></div>
          ) : (
            <div className="divide-y divide-gray-50">
              {auditLogs.map((log) => (
                <div key={log.id} className={`px-6 py-4 hover:bg-gray-50 transition-colors flex items-start gap-4 ${log.severity === 'critical' ? 'bg-red-50/50' : log.severity === 'sensitive' ? 'bg-amber-50/30' : ''}`}>
                  <div className={`w-2 h-2 rounded-full flex-shrink-0 mt-2 ${log.severity === 'critical' ? 'bg-red-500' : log.severity === 'sensitive' ? 'bg-amber-500' : 'bg-emerald-500'}`} />
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-3 mb-1 flex-wrap">
                      <span className="font-mono text-xs font-bold text-navy-700 bg-navy-100 px-2 py-0.5 rounded">{log.action}</span>
                      <span className="text-xs font-semibold text-navy-800">{log.actor_email}</span>
                      <span className="text-xs text-gray-500">→ {log.target_ref}</span>
                      {log.severity === 'critical' && <span className="text-[10px] font-bold text-red-600 bg-red-100 px-1.5 py-0.5 rounded-full">CRITIQUE</span>}
                      {log.severity === 'sensitive' && <span className="text-[10px] font-bold text-amber-600 bg-amber-100 px-1.5 py-0.5 rounded-full">SENSIBLE</span>}
                    </div>
          ...
          <div className="px-6 py-4 border-t border-gray-100 bg-gray-50">
            <p className="text-[10px] text-gray-400">Les journaux d'audit sont immuables et en ajout seul. Aucun enregistrement ne peut être modifié ou supprimé.</p>
          </div>
        </div>
      )}
      ...
      {activeMainTab === 'partners' && (
        <div className="space-y-4">
          ...
          <div className="bg-white border border-gray-200 rounded-2xl p-6 text-center">
            ...
            <p className="text-navy-900 font-semibold mb-2">{t('Répertoire Partenaires', 'Partner Directory')}</p>
            <p className="text-gray-500 text-sm mb-4">{t('Gérez le répertoire complet des partenaires depuis la page dédiée.', 'Manage the complete partner directory from the dedicated page.')}</p>
            <a href="/admin/partners" className="inline-flex items-center gap-2 px-5 py-2.5 bg-navy-900 hover:bg-navy-700 text-white text-sm font-semibold rounded-xl transition-all duration-200">
              <Network size={14} />{t('Accéder au répertoire partenaires', 'Access partner directory')}
            </a>
          </div>
          <div className="bg-white border border-gray-200 rounded-2xl p-6">
            ...
            <p className="text-gray-500 text-sm mb-4">{t('Gérez les assignations de dossiers aux partenaires agréés.', 'Manage case assignments to approved partners.')}</p>
            <a href="/admin/partner-assignments" className="inline-flex items-center gap-2 px-4 py-2 border border-gray-200 text-gray-700 text-sm font-medium rounded-xl hover:bg-gray-50 transition-colors">
              <ClipboardList size={14} />{t('Voir les assignations', 'View assignments')}
            </a>
          </div>
        </div>
      )}
