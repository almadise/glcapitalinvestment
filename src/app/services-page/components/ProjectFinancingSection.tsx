...
export default function ProjectFinancingSection() {
  ...
  const eligibilityCriteria = [
    ...
    t(
      ...
      'Identified project sponsors with demonstrated execution capacity'
    ),
    t(
      'Secteur viable : énergie, infrastructure, immobilier, agro-industriel, télécom',
      'Viable sector: energy, infrastructure, real estate, agri-industrial, telecom'
    ),
    t(
      'Juridiction bancable (pays non sanctionnés uniquement)',
      'Bankable jurisdiction (non-sanctioned countries only)'
    ),
    ...
    t(
      ...
      'Feasibility study or business plan available'
    ),
  ];
  ...
  const requiredDocs = [
    {
      id: 'pf-exec',
      docFr: 'Résumé exécutif',
      docEn: 'Executive Summary',
      required: true,
      format: 'PDF, 2–5 pages',
    },
    {
      id: 'pf-bp',
      docFr: 'Business Plan / Étude de faisabilité',
      docEn: 'Business Plan / Feasibility Study',
      required: true,
      format: 'PDF or DOCX',
    },
    {
      id: 'pf-fin',
      docFr: 'Modèle financier avec hypothèses',
      docEn: 'Financial Model with assumptions',
      required: true,
      format: 'Excel (.xlsx)',
    },
    {
      id: 'pf-capex',
      docFr: 'Calendrier Capex / Opex détaillé',
      docEn: 'Detailed Capex / Opex schedule',
      required: true,
      format: 'Excel or PDF',
    },
    {
      id: 'pf-kyc',
      docFr: 'KYC porteur + Registre des sociétés',
      docEn: 'Sponsor KYC + Company Registry',
      required: true,
      format: 'PDF',
    },
    {
      id: 'pf-fin-stmts',
      docFr: 'États financiers (2–3 ans)',
      docEn: 'Financial statements (2–3 years)',
      required: false,
      format: t('Si entité existante', 'If existing entity'),
    },
    {
      id: 'pf-contracts',
      docFr: 'Contrats clés (off-take, EPC, licences)',
      docEn: 'Key contracts (off-take, EPC, licenses)',
      required: false,
      format: t('Si disponible', 'If available'),
    },
    {
      id: 'pf-equity',
      docFr: 'Preuve de fonds propres',
      docEn: 'Proof of equity / skin in the game',
      required: false,
      format: t('Relevés bancaires ou engagement', 'Bank statements or commitment'),
    },
  ];
  ...
  const useCases = [
    {
      id: 'uc-solar',
      titleFr: 'Énergie renouvelable (Solaire, Éolien, Hydro)',
      titleEn: 'Renewable Energy (Solar, Wind, Hydro)',
      descFr: 'Structuration IPP, financement adossé PPA',
      descEn: 'IPP structuring, PPA-backed financing',
    },
    {
      id: 'uc-infra',
      titleFr: 'Infrastructure & Transport',
      titleEn: 'Infrastructure & Transport',
      descFr: 'Routes, ports, aéroports, services publics',
      descEn: 'Roads, ports, airports, utilities',
    },
    {
      id: 'uc-re',
      titleFr: 'Immobilier commercial',
      titleEn: 'Commercial Real Estate',
      descFr: 'Bureaux, commerce, développement mixte',
      descEn: 'Office, retail, mixed-use development',
    },
    {
      id: 'uc-agri',
      titleFr: 'Agro-industriel',
      titleEn: 'Agri-Industrial',
      descFr: 'Usines de transformation, irrigation, chaîne du froid',
      descEn: 'Processing plants, irrigation, cold chain',
    },
    {
      id: 'uc-mining',
      titleFr: 'Mines & Ressources',
      titleEn: 'Mining & Resources',
      descFr: "Financement de l'exploration à la production",
      descEn: 'Exploration to production financing',
    },
    {
      id: 'uc-telecom',
      titleFr: 'Télécom & Infrastructure numérique',
      titleEn: 'Telecom & Digital Infrastructure',
      descFr: 'Tours, fibre, centres de données',
      descEn: 'Towers, fiber, data centers',
    },
  ];
  ...
  const notEligible = [
    ...
    t(
      ...
      'No identified sponsor or equity contribution'
    ),
  ...
  const timeline = [
    {
      step: '01',
      labelFr: 'Pré-qualification',
      labelEn: 'Pre-qualification',
      durationFr: '1–3 jours',
      durationEn: '1–3 days',
    },
    {
      step: '02',
      labelFr: 'Revue KYC/AML',
      labelEn: 'KYC/AML Review',
    ...
    {
      step: '04',
      labelFr: 'Soumission institutionnelle',
      labelEn: 'Institutional Submission',
      durationFr: 'Variable',
      durationEn: 'Variable',
    },
  ];
      ...
      <div className="max-w-screen-2xl mx-auto px-6 lg:px-10">
            ...
            <div className="space-y-3">
              {useCases?.map((uc) => (
                ...
                >
                  ...
                  />
                  <div>
                    <p className="text-sm font-semibold" style={{ color: '#1E2D4A' }}>
                      {t(uc?.titleFr, uc?.titleEn)}
                    </p>
                    <p className="text-xs" style={{ color: '#4A5C7A' }}>
                      {t(uc?.descFr, uc?.descEn)}
                    </p>
                  </div>
                </div>
              ))}
          ...
          <div>
            ...
            <div className="space-y-2 mb-6">
              {requiredDocs?.map((doc) => (
                <div
                  key={doc?.id}
                  className="flex items-start gap-2 p-2.5 rounded-lg transition-colors hover:bg-gray-50"
                >
                  <FileText
                    size={13}
                    className={`flex-shrink-0 mt-0.5 ${doc?.required ? 'text-navy-600' : 'text-gray-400'}`}
                  />
                  <div>
                    <p
                      className={`text-xs font-medium ${doc?.required ? 'text-navy-800' : 'text-gray-500'}`}
                    >
        ...
        <div className="mt-10 p-6 rounded-2xl" style={{ background: '#1E2D4A' }}>
          <p
            ...
            style={{ color: '#B8912A' }}
          >
            {t('Calendrier type', 'Typical Timeline')}
          </p>
              ...
              <React.Fragment key={`pf-timeline-${s?.step}`}>
                <div className="flex-1 text-center">
                  <span
                    ...
                    style={{ color: 'rgba(184,145,42,0.3)' }}
                  >
                    {s?.step}
                  </span>
                  <p className="text-white text-sm font-semibold">{t(s?.labelFr, s?.labelEn)}</p>
                  <p className="text-white/40 text-xs font-mono">
                    {t(s?.durationFr, s?.durationEn)}
                  </p>
                </div>
                {i < 3 && (
                  <div className="hidden sm:flex items-center" style={{ color: '#4A5C7A' }}>
                    →
                  </div>
                )}
              </React.Fragment>
