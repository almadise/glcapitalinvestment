...
const navItems: NavItem[] = [
  { id: 'nav-overview', icon: LayoutDashboard, labelFr: 'Vue d\'ensemble', labelEn: 'Overview', href: '/analyst-dashboard' },
  { id: 'nav-cases', icon: ClipboardList, labelFr: 'Dossiers à traiter', labelEn: 'Cases to Process', href: '/analyst-dashboard/cases' },
  { id: 'nav-analytics', icon: BarChart3, labelFr: 'Analytique', labelEn: 'Analytics', href: '/admin/analytics' },
  { id: 'nav-reports', icon: FileSearch, labelFr: 'Rapports', labelEn: 'Reports', href: '/analyst-dashboard/reports' },
];
  ...
  const sidebarContent = (isMobile = false) => (
    <>
      <nav className="flex-1 py-4 px-2 overflow-y-auto scrollbar-thin">
        {(!collapsed || isMobile) && <p className="section-label mb-1.5 px-2">{lang === 'fr' ? 'Analyse' : 'Analysis'}</p>}
        ...
        <div className={`flex items-center h-16 border-b border-white/5 px-3 flex-shrink-0 ${collapsed ? 'justify-center' : 'gap-3'}`}>
          ...
          {!collapsed && (
            <div className="overflow-hidden">
              ...
              <span className="text-gold text-[9px] font-medium tracking-widest uppercase whitespace-nowrap">{lang === 'fr' ? 'Analyste' : 'Analyst'}</span>
            </div>
          )}
        </div>
  ...
  return (
    <RoleGuard allowedRoles={['analyst', 'admin']}>
      <div className="min-h-screen bg-slate-50 flex">
        ...
        <AnalystSidebar collapsed={collapsed} mobileOpen={mobileOpen} onCloseMobile={() => setMobileOpen(false)} onToggleCollapse={() => setCollapsed(!collapsed)} />
        ...
        <div className={`flex-1 flex flex-col min-w-0 transition-all duration-300 ${collapsed ? 'lg:ml-16' : 'lg:ml-60'}`}>
          <header className="h-16 bg-white border-b border-slate-200 flex items-center px-4 sm:px-6 gap-4 flex-shrink-0 sticky top-0 z-20">
            ...
            <div className="flex items-center gap-2">
              ...
              <span className="font-semibold text-navy text-sm">{lang === 'fr' ? 'Espace Analyste' : 'Analyst Portal'}</span>
            </div>
