...
export default function AnalystCasesPage() {
  ...
  const handleStatusUpdate = async () => {
    ...
    try {
      ...
      const updatedCase = {
        ...
        closure_reason: updateData.closure_reason,
      };
  ...
  const handleSaveTags = async () => {
    ...
    try {
      const { error } = await supabase
        .from('case_files')
        .update({ risk_tags: selectedTags })
        .eq('id', selectedCase.id);
      if (error) throw error;
      setSelectedCase({ ...selectedCase, risk_tags: selectedTags });
      setCases((prev) =>
        prev.map((c) => (c.id === selectedCase.id ? { ...c, risk_tags: selectedTags } : c))
      );
  ...
  const filtered = cases.filter((c) => {
    ...
    const matchTags =
      filters.riskTags.length === 0 || filters.riskTags.every((t) => c.risk_tags?.includes(t));
      ...
      <div className="mb-4 sm:mb-6 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div>
          ...
          <p className="text-slate-500 text-sm">
            Traitement, messages, notes internes et gestion des statuts
          </p>
        </div>
        <button
          onClick={fetchCases}
          className="flex items-center gap-2 px-3 py-2 bg-white border border-slate-200 rounded-lg text-sm text-slate-600 hover:bg-slate-50 transition-colors self-start sm:self-auto"
        >
                  ...
                  {c.risk_tags && c.risk_tags.length > 0 && (
                    <div className="flex flex-wrap gap-1 mt-2">
                      {c.risk_tags.slice(0, 2).map((t) => (
                        <span
                          key={t}
                          className="inline-flex items-center gap-1 px-1.5 py-0.5 bg-red-50 border border-red-200 text-red-700 text-xs rounded"
                        >
                          <AlertTriangle size={9} />
                          {t.split(' ').slice(0, 2).join(' ')}
                        </span>
                      ))}
                      {c.risk_tags.length > 2 && (
                        <span className="text-xs text-slate-400">+{c.risk_tags.length - 2}</span>
                      )}
                    </div>
                  )}
                </div>
          ...
          <div className="w-full max-w-2xl bg-white shadow-2xl flex flex-col overflow-hidden">
            ...
            <div className="flex items-center justify-between px-4 sm:px-6 py-4 border-b border-slate-200 bg-slate-50">
              <div className="flex-1 min-w-0 pr-3">
                <h2 className="font-bold text-navy text-sm sm:text-base truncate">
              ...
              <button
                ...
                className="text-slate-400 hover:text-navy p-1.5 rounded-lg hover:bg-slate-200 flex-shrink-0"
              >
                <X size={18} />
              </button>
            </div>
                    ...
                    <div className="space-y-2">
                      ...
                      {newStatus === 'REJETE' && (
                        <textarea
                          ...
                          rows={2}
                          className="w-full px-3 py-2 border border-red-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-red-500/20 resize-none bg-red-50"
                        />
                      )}
                      {newStatus === 'CLOTURE' && (
                        <textarea
                          ...
                          rows={2}
                          className="w-full px-3 py-2 border border-teal-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-teal-500/20 resize-none bg-teal-50"
                        />
                      )}
                      <button
                  ...
                  {tab === 'notes' ? (
                    ...
                    <span className="flex items-center gap-1.5">
                      ...
                      {statusHistory.length > 0 && (
                        <span className="bg-slate-100 text-slate-600 text-xs px-1.5 py-0.5 rounded-full font-semibold">
              ...
              {detailLoading ? (
                ...
                <div className="space-y-3">
                  <div className="bg-blue-50 border border-blue-200 rounded-lg p-3 text-xs text-blue-700">
                    <strong>Documents du dossier :</strong> Téléversez des pièces justificatives,
                    KYC, contrats ou tout document lié à ce dossier.
                  </div>
                  <UploadWidget caseId={selectedCase.id} onUploadComplete={() => {}} />
                </div>
              ) : activeTab === 'notes' ? (
                <div className="space-y-4">
                  ...
                  <div className="bg-slate-50 border border-slate-200 rounded-xl p-3">
                    <label className="block text-xs font-semibold text-slate-600 mb-2 flex items-center gap-1.5">
                      <Plus size={11} /> Nouvelle note interne
                    </label>
                    <div className="flex gap-2">
                      <textarea
                        value={noteContent}
                        onChange={(e) => setNoteContent(e.target.value)}
                        placeholder="Ajouter une note interne (non visible client)..."
                        rows={3}
                        className="flex-1 px-3 py-2 border border-slate-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500/20 resize-none bg-white"
                      />
                      <button
                  ...
                  <div className="flex items-center gap-1.5 text-xs text-slate-400">
                    <MessageSquare size={11} /> Notes internes - non visibles par le client
                  </div>
                  {internalNotes.length === 0 ? (
                        ...
                        <div className="flex items-start justify-between mb-2">
                          <div className="flex items-center gap-2 flex-wrap">
                            ...
                            <span className="flex items-center gap-1 text-xs font-medium text-slate-700">
                              <User size={10} /> {note.author_email || 'Anonyme'}
                            </span>
                            <span className="flex items-center gap-1 text-xs text-slate-400">
                              <Clock size={10} /> {formatDate(note.created_at)}
                            </span>
                            {note.updated_at && note.updated_at !== note.created_at && (
                              <span className="text-xs text-slate-400 italic">(modifiée)</span>
                            )}
                          </div>
                        ...
                        {editingNoteId === note.id ? (
                          <div className="space-y-2">
                            <textarea
                              ...
                              rows={3}
                              className="w-full px-3 py-2 border border-blue-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500/20 resize-none bg-white"
                            />
                            <div className="flex items-center gap-2">
                              ...
                              <button
                                ...
                                className="px-3 py-1.5 text-slate-600 hover:text-navy text-xs font-medium"
                              >
                                Annuler
                              </button>
                            </div>
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <div className="flex items-center gap-3 mt-1.5 flex-wrap">
                            <span className="flex items-center gap-1 text-xs text-slate-500 font-medium">
                              <User size={10} /> {h.changed_by_email || 'Système'}
                            </span>
                            <span className="flex items-center gap-1 text-xs text-slate-400">
                              <Clock size={10} /> {formatDate(h.created_at)}
                            </span>
                          </div>
                        </div>
                      </div>
                ...
                <div className="space-y-4">
                  <div className="bg-red-50 border border-red-200 rounded-lg p-3 text-xs text-red-700">
                    <strong>Tags risque :</strong> Ces tags sont internes et non visibles par le
                    client.
                  </div>
                  <p className="text-xs text-slate-500">
                    Sélectionnez les tags de risque applicables à ce dossier :
                  </p>
                  <div className="flex flex-wrap gap-2">
                    ...
                    <div className="bg-white border border-slate-200 rounded-lg p-3">
                      ...
                      <div className="flex flex-wrap gap-1.5">
                        {selectedTags.map((tag) => (
                          <span
                            key={tag}
                            className="inline-flex items-center gap-1 px-2 py-1 bg-red-100 border border-red-200 text-red-700 text-xs rounded-lg font-medium"
                          >
                  ...
                  <button
                    ...
                    disabled={savingTags}
                    className="flex items-center gap-2 px-4 py-2 bg-navy text-white rounded-lg text-sm font-semibold hover:bg-navy/90 disabled:opacity-50 transition-colors"
                  >
                    {savingTags ? (
                      <Loader2 size={14} className="animate-spin" />
                    ) : (
