...
export default function AdminPartnersPage() {
  ...
  const fetchSubmissions = useCallback(async () => {
    ...
    try {
      ...
      const enriched = await Promise.all(
        (data || []).map(async (s: SubmissionEntry) => {
  ...
  useEffect(() => {
    if (activeTab === 'journal' && partners.length >= 0) fetchSubmissions();
  ...
  const handleEdit = (p: Partner) => {
    setForm({
      ...
      zones: p.zones?.join(', ') || '',
      criteria: p.criteria || '',
      internal_contact: p.internal_contact || '',
  ...
  const handleSubmit = async () => {
    if (!submitForm.partner_id || !submitForm.case_id) {
      toast.error('Partenaire et dossier requis');
      return;
    }
    setSubmitting(true);
    try {
  ...
  return (
    <AdminLayout>
      <div className="mb-6 flex items-center justify-between">
        <div>
          ...
          <p className="text-slate-500 text-sm">
            Gestion confidentielle des partenaires - non exposé côté client
          </p>
        </div>
      ...
      <div className="bg-amber-50 border border-amber-200 rounded-xl p-3 flex items-center gap-3 mb-4 text-xs text-amber-800">
        ...
        <span>
          <strong>Confidentiel :</strong> Ce répertoire est strictement réservé au back-office. Côté
          portail client, seul le message "dossier soumis à une institution agréée" est affiché.
        </span>
      </div>
      ...
      <div className="flex gap-1 bg-slate-100 p-1 rounded-xl mb-4 w-fit">
        <button
          ...
          className={`flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-semibold transition-all ${activeTab === 'partners' ? 'bg-white text-navy shadow-sm' : 'text-slate-500 hover:text-navy'}`}
        >
          ...
          <div className="bg-white rounded-xl border border-slate-200 p-4 mb-4">
            <div className="relative">
              ...
              <input
                ...
                className="w-full pl-9 pr-3 py-2 border border-slate-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-gold/20"
              />
            </div>
          </div>
              ...
              {filtered.length === 0 ? (
                ...
                filtered.map((p) => {
                  ...
                  return (
                    <div
                      key={p.id}
                      className={`bg-white rounded-xl border p-5 transition-all ${p.is_active ? 'border-slate-200 hover:border-gold/40 hover:shadow-md' : 'border-slate-100 opacity-60'}`}
                    >
                      <div className="flex items-start justify-between mb-3">
                        ...
                        <div className="flex items-center gap-1">
                          <button
                            ...
                            className="p-1.5 text-slate-400 hover:text-navy hover:bg-slate-100 rounded-lg"
                          >
                            <Edit2 size={13} />
                          </button>
                          <button
                            onClick={() => handleToggleActive(p)}
                            className={`p-1.5 rounded-lg ${p.is_active ? 'text-emerald-600 hover:bg-emerald-50' : 'text-slate-400 hover:bg-slate-100'}`}
                          >
                            <Check size={13} />
                          </button>
                        </div>
                      ...
                      {p.internal_contact && (
                        <div className="text-xs text-slate-500 mb-1">
                          <span className="font-medium">Contact interne :</span>{' '}
                          {p.internal_contact}
                        </div>
                      )}
                      {p.contact_email && (
                      ...
                      {!p.is_active && (
                        <div className="mt-2 text-xs text-slate-400 italic">Partenaire inactif</div>
                      )}
                    </div>
                  );
                })
          ...
          <div className="px-4 py-3 border-b border-slate-200 bg-slate-50 flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <span className="text-sm font-semibold text-navy">
                Journal de soumission partenaires
              </span>
            </div>
              ...
              <table className="w-full text-sm">
                ...
                <tbody className="divide-y divide-slate-100">
                  {submissions.length === 0 ? (
                    ...
                    submissions.map((s) => {
                      const statusCfg =
                        SUBMISSION_STATUS[s.status || 'soumis'] || SUBMISSION_STATUS.soumis;
                      return (
                        <tr key={s.id} className="hover:bg-slate-50 transition-colors">
                          ...
                          <td className="px-4 py-3">
                            <span
                              className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs font-semibold ${statusCfg.bg} ${statusCfg.color}`}
                            >
                              {statusCfg.label}
                            </span>
                          </td>
            ...
            <div className="px-6 py-4 space-y-4">
              ...
              <div>
                <label className="block text-xs font-semibold text-slate-600 mb-1">
                  Zones géographiques (séparées par virgule)
                </label>
                <input
                  value={form.zones}
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => setForm({ ...form, internal_contact: e.target.value })}
          ...
          <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-md">
            ...
            <div className="px-6 py-4 space-y-4">
              <div>
                ...
                <div className="relative">
                  <select
                    value={submitForm.partner_id}
                    onChange={(e) => setSubmitForm({ ...submitForm, partner_id: e.target.value })}
                    className="w-full appearance-none px-3 pr-8 py-2 border border-slate-200 rounded-lg text-sm focus:outline-none bg-white"
                  >
                    ...
                    {partners
                      ...
                      .map((p) => (
                        <option key={p.id} value={p.id}>
                          {p.name} ({PARTNER_TYPES[p.type]?.label || p.type})
                        </option>
                      ))}
                  </select>
                  <ChevronDown
              ...
              <div>
                ...
                <div className="relative">
                  <select
                    value={submitForm.case_id}
                    onChange={(e) => setSubmitForm({ ...submitForm, case_id: e.target.value })}
                    className="w-full appearance-none px-3 pr-8 py-2 border border-slate-200 rounded-lg text-sm focus:outline-none bg-white"
                  >
              ...
              <div>
                ...
                <textarea
                  ...
                  rows={3}
                  className="w-full px-3 py-2 border border-slate-200 rounded-lg text-sm focus:outline-none resize-none"
                  placeholder="Conditions, remarques, contacts..."
                />
              </div>
              <div className="bg-amber-50 border border-amber-200 rounded-lg p-2.5 text-xs text-amber-700">
                <FileText size={11} className="inline mr-1" />
                Cette entrée sera enregistrée dans le journal confidentiel. Le client ne verra que
                "soumis à une institution agréée".
              </div>
            </div>
            <div className="flex items-center justify-end gap-3 px-6 py-4 border-t border-slate-200">
              <button
              ...
              <button
                ...
                disabled={submitting || !submitForm.partner_id || !submitForm.case_id}
                className="flex items-center gap-2 px-4 py-2 bg-navy text-white rounded-lg text-sm font-semibold hover:bg-navy/90 disabled:opacity-50"
              >
