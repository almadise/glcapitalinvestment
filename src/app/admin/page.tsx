...
export default function AdminDashboardPage() {
  ...
  const refreshStats = useCallback(async () => {
    ...
    const avgDays =
      processed.length > 0
        ? Math.round(
            processed.reduce((acc: number, c: any) => {
              const diff =
    ...
    const recent = (allCases.data || []).filter(
      (c: any) => new Date(c.created_at) >= ninetyDaysAgo
    );
    ...
    const score =
      recent.length > 0 ? Math.round(((recent.length - recentFlagged) / recent.length) * 100) : 100;
    setScorecard({
  ...
  const t = {
    ...
    subtitle:
      lang === 'fr'
        ? "Vue d'ensemble complète du portail GL Capital"
        : 'Complete overview of the GL Capital portal',
  ...
  const kpis = [
    ...
    {
      label: lang === 'fr' ? 'Dossiers actifs' : 'Active Cases',
      value: stats.cases,
      icon: ClipboardList,
      color: 'text-blue-600',
      bg: 'bg-blue-50',
    ...
    {
      label: lang === 'fr' ? 'Dossiers en attente' : 'Pending Cases',
    ...
    {
      label: lang === 'fr' ? 'Flags conformité' : 'Compliance Flags',
    ...
    {
      label: lang === 'fr' ? 'Dossiers rejetés' : 'Rejected Cases',
  ...
  const quickLinks = [
    {
      label: lang === 'fr' ? 'Voir les soumissions' : 'View Submissions',
      href: '/admin/contact-submissions',
      icon: Inbox,
      desc: lang === 'fr' ? 'Gérer les demandes de contact' : 'Manage contact requests',
    },
    {
      label: lang === 'fr' ? 'Gestion dossiers' : 'Case Management',
      href: '/admin/case-management',
      icon: ClipboardList,
      desc: lang === 'fr' ? 'Suivre et mettre à jour les dossiers' : 'Track and update cases',
    },
    {
      label: lang === 'fr' ? 'Gérer les utilisateurs' : 'Manage Users',
      href: '/admin/users',
      icon: Users,
      desc: lang === 'fr' ? 'Modifier les rôles et accès' : 'Edit roles and access',
    },
    {
      label: lang === 'fr' ? 'Analytique avancée' : 'Advanced Analytics',
      href: '/admin/analytics',
      icon: BarChart3,
      desc: lang === 'fr' ? 'Statistiques et graphiques' : 'Statistics and charts',
    },
  ];
      ...
      {loading ? (
        ...
        <>
          ...
          <div
            ...
            role="region"
            aria-label={lang === 'fr' ? 'Indicateurs clés' : 'Key performance indicators'}
          >
            {kpis.map((kpi) => (
              <Link
                key={kpi.label}
                href={kpi.href}
                className="bg-white rounded-xl border border-slate-200 p-4 sm:p-5 hover:border-gold/40 hover:shadow-md transition-all group"
            ...
            >
                ...
                <ResponsiveContainer width="100%" height={200}>
                  ...
                  >
                    ...
                    <Line
                      ...
                      strokeWidth={2}
                      dot={{ fill: '#ef4444', r: 4 }}
                      name={lang === 'fr' ? 'Flags conformité' : 'Compliance Flags'}
                    />
                  </LineChart>
                </ResponsiveContainer>
              )}
              <div className="flex items-center gap-4 mt-3">
                <div className="flex items-center gap-1.5">
                  <div className="w-3 h-3 rounded-full bg-gold" aria-hidden="true" />
                ...
                <div className="flex items-center gap-1.5">
                  ...
                  <span className="text-xs text-slate-500">
                    {lang === 'fr' ? 'Flags conformité' : 'Compliance Flags'}
                  </span>
                </div>
              </div>
            </div>
              ...
              {scorecard ? (
                <>
                  <div className="text-center mb-4">
                    <div
                      className={`text-5xl font-bold ${scorecardColor}`}
                      aria-label={`${t.complianceScore}: ${scorecard.score}%`}
                    >
                      {scorecard.score}%
                    </div>
                  ...
                  <div
                    ...
                    aria-valuemin={0}
                    aria-valuemax={100}
                    aria-label={t.complianceScore}
                  >
                    <div
                      className={`h-2 rounded-full transition-all ${scorecard.score >= 80 ? 'bg-emerald-500' : scorecard.score >= 60 ? 'bg-amber-500' : 'bg-red-500'}`}
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              {quickLinks.map((link) => (
                <Link
                  key={link.label}
                  href={link.href}
                  className="bg-white rounded-xl border border-slate-200 p-4 flex items-center gap-3 sm:gap-4 hover:border-gold/40 hover:shadow-md transition-all group"
                >
                  ...
                  <ArrowRight
                    size={16}
                    className="text-slate-400 group-hover:text-gold transition-colors flex-shrink-0"
                    aria-hidden="true"
                  />
                </Link>
              ))}
