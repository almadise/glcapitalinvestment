...
export default function AdminAnalyticsDashboard() {
  ...
  const t = {
    ...
    subtitle:
      lang === 'fr'
        ? 'Pipeline dossiers, conformité et métriques de performance'
        : 'Dossier pipeline, compliance and performance metrics',
    ...
    complianceBreakdown:
      lang === 'fr' ? 'Répartition flags conformité' : 'Compliance Flag Breakdown',
    avgProcessing:
      lang === 'fr' ? 'Délai moyen par statut (jours)' : 'Avg Processing Time by Status (days)',
  ...
  const fetchAnalytics = useCallback(async () => {
    ...
    try {
      ...
      const [submissionsRes, casesRes, usersRes, caseStatusRes, caseTypeRes, allCasesRes] =
        await Promise.all([
      ...
      for (let i = 5; i >= 0; i--) {
        ...
        const key = d.toLocaleDateString(lang === 'fr' ? 'fr-FR' : 'en-US', {
          month: 'short',
          year: '2-digit',
        });
        monthCounts[key] = 0;
      ...
      const submissionsByMonth = Object.entries(monthCounts).map(([month, count]) => ({
        month,
        count,
      }));

      // Pipeline trend (received, eligible, rejected per month)
      const pipelineMonths: Record<
        string,
        { received: number; eligible: number; rejected: number }
      > = {};
      for (let i = 5; i >= 0; i--) {
        const d = new Date(now.getFullYear(), now.getMonth() - i, 1);
        const key = d.toLocaleDateString(lang === 'fr' ? 'fr-FR' : 'en-US', {
      ...
      const complianceFlagsByMonth = Object.entries(flagMonths).map(([month, v]) => ({
        month,
        ...v,
      }));
      ...
      const funnelData = [
        {
          stage: lang === 'fr' ? 'Soumissions contacts' : 'Contact submissions',
          count: totalSub,
          rate: 100,
        },
        {
          stage: lang === 'fr' ? 'Dossiers créés' : 'Files created',
          count: totalCasesCount,
          rate: totalSub > 0 ? Math.round((totalCasesCount / totalSub) * 100) : 0,
        },
        {
          stage: lang === 'fr' ? 'Éligibles' : 'Eligible',
          count: eligibleCount,
          rate: totalCasesCount > 0 ? Math.round((eligibleCount / totalCasesCount) * 100) : 0,
        },
        {
          stage: lang === 'fr' ? 'Clôturés' : 'Closed',
          count: closedCount,
          rate: eligibleCount > 0 ? Math.round((closedCount / eligibleCount) * 100) : 0,
        },
      ];
  ...
  const handleExportCSV = async (type: 'dossiers' | 'history' | 'notes' | 'risk_tags') => {
    ...
    try {
      ...
      if (type === 'dossiers') {
        ...
        csvContent =
          'ID,Titre,Type,Statut,Client Email,Tags Risque,Motif Rejet,Motif Clôture,Créé le,Mis à jour\n';
        (cases || []).forEach((c: any) => {
        ...
        const { data: cases } = await supabase
          .from('case_files')
          .select('id, ref, project_name, status, risk_tags, client_email, created_at')
          .not('risk_tags', 'is', null);
        csvContent = 'Dossier ID,Titre,Statut,Client,Tags Risque,Date\n';
        (cases || [])
          .filter((c: any) => c.risk_tags && c.risk_tags.length > 0)
          .forEach((c: any) => {
    ...
    <AdminLayout>
      <div className="space-y-6">
        ...
        <div
          ...
          role="region"
          aria-label="Exports audit et conformité"
        >
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
            {[
              {
                key: 'dossiers' as const,
                label: 'Historique dossiers',
                desc: 'Tous les dossiers avec statuts et tags',
                icon: FolderOpen,
                color: 'text-blue-600',
                bg: 'bg-blue-50',
              },
              {
                ...
                desc: 'Qui a changé quoi, quand et pourquoi',
                icon: TrendingUp,
              ...
              },
              {
                ...
                desc: 'Toutes les notes avec auteur et horodatage',
                icon: FileText,
                color: 'text-purple-600',
                bg: 'bg-purple-50',
              },
              {
                key: 'risk_tags' as const,
                label: 'Journal tags risque',
                desc: 'Dossiers avec tags pays/secteur/sanctions',
                icon: Tag,
                color: 'text-red-600',
                bg: 'bg-red-50',
              },
        ...
        {error && (
          <div
            className="flex items-center gap-3 bg-red-50 border border-red-200 rounded-xl p-4 text-red-600 text-sm"
            role="alert"
          >
        ...
        {loading ? (
          ...
          <>
            ...
            <div
              ...
              role="region"
              aria-label={lang === 'fr' ? 'Indicateurs clés' : 'Key metrics'}
            >
              {[
                {
                ...
                <div key={kpi.label} className="bg-white border border-slate-200 rounded-2xl p-5">
                  <div className="flex items-center justify-between mb-3">
                    <div
                      className={`w-10 h-10 rounded-xl border flex items-center justify-center ${kpi.bg}`}
                    >
                ...
                <ResponsiveContainer width="100%" height={220}>
                  ...
                  >
                    ...
                    <Area
                      ...
                      strokeWidth={2}
                      name={lang === 'fr' ? 'Éligibles' : 'Eligible'}
                    />
                    <Area
                      type="monotone"
                      dataKey="rejected"
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              ...
              <div
                className="bg-white border border-slate-200 rounded-2xl p-5"
                role="region"
                aria-label={t.complianceBreakdown}
              >
                <div className="flex items-center gap-2 mb-4">
                ...
                {data.complianceFlagsByMonth.every((d) => d.flagged === 0 && d.clean === 0) ? (
                  ...
                  <ResponsiveContainer width="100%" height={200}>
                    <BarChart
                      data={data.complianceFlagsByMonth}
                      margin={{ top: 0, right: 0, left: -20, bottom: 0 }}
                    >
                      <CartesianGrid strokeDasharray="3 3" stroke="rgba(0,0,0,0.05)" />
                      <XAxis
            ...
            {data.avgProcessingByStatus.length > 0 && (
              ...
              >
                ...
                <ResponsiveContainer width="100%" height={180}>
                  <BarChart
                    data={data.avgProcessingByStatus}
                    layout="vertical"
                  ...
                  >
                    ...
                    <Tooltip
                      ...
                      formatter={(v) => [
                        `${v} ${lang === 'fr' ? 'jours' : 'days'}`,
                        lang === 'fr' ? 'Délai moyen' : 'Avg time',
                      ]}
                    />
                    <Bar
                      dataKey="avgDays"
            ...
            {data.funnelData && data.funnelData.length > 0 && (
              <div
                className="bg-white border border-slate-200 rounded-2xl p-5"
                role="region"
                aria-label={lang === 'fr' ? 'Entonnoir de conversion' : 'Conversion funnel'}
              >
                <div className="flex items-center gap-2 mb-4">
                  <TrendingUp size={16} className="text-emerald-500" aria-hidden="true" />
                  <h2 className="text-navy font-semibold">
                    {lang === 'fr' ? 'Entonnoir lead → closing' : 'Lead → closing funnel'}
                  </h2>
                </div>
            ...
            {data.acquisitionBySource && data.acquisitionBySource.length > 0 && (
              <div
                className="bg-white border border-slate-200 rounded-2xl p-5"
                role="region"
                aria-label={lang === 'fr' ? 'Acquisition par canal' : 'Acquisition by channel'}
              >
                <div className="flex items-center gap-2 mb-4">
                  <Users size={16} className="text-blue-500" aria-hidden="true" />
                  <h2 className="text-navy font-semibold">
                    {lang === 'fr'
                      ? 'Acquisition par canal (utm_source)'
                      : 'Acquisition by channel (utm_source)'}
                  </h2>
                </div>
                  ...
                  {data.acquisitionBySource.map((item, idx) => {
                    ...
                    return (
                      <div key={item.source} className="flex items-center gap-3">
                        <span className="text-xs text-slate-500 w-28 truncate flex-shrink-0 text-right capitalize">
                          {item.source}
                        </span>
                        ...
                        <span className="text-xs font-bold text-navy w-8 text-right flex-shrink-0">
                          {item.count}
                        </span>
                      </div>
                    );
