...
const TASKS: ReadinessTask[] = [
  {
    ...
    description:
      'Vérification des enregistrements DNS, propagation et configuration des sous-domaines.',
    status: 'warning',
    details: [
  ...
  },
  {
    ...
    description:
      'Consentement cookies, politique de confidentialité et gestion des préférences utilisateur.',
    status: 'warning',
    details: [
  ...
  },
  {
    ...
    description:
      'Localisation des données, conformité RGPD sur le transfert et la souveraineté des données.',
    status: 'pending',
    details: [
        ...
        <div className="flex items-start gap-4">
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-start justify-between gap-3 flex-wrap">
              ...
              <span
                className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-semibold ${cfg.bg} ${cfg.border} ${cfg.color} border flex-shrink-0`}
              >
              ...
              <ul className="space-y-2" role="list">
                {task.details.map((detail, i) => (
                  <li key={i} className="flex items-start gap-2 text-sm text-slate-600">
                    <span className="text-slate-400 mt-1 flex-shrink-0" aria-hidden="true">
                      •
                    </span>
                    {detail}
                  </li>
              ...
              <ol className="space-y-2" role="list">
                {task.actionItems.map((action, i) => (
                  <li key={i} className="flex items-start gap-2.5 text-sm text-slate-700">
                    <span
                      className="w-5 h-5 rounded-full bg-navy/10 text-navy text-[10px] font-bold flex items-center justify-center flex-shrink-0 mt-0.5"
                      aria-hidden="true"
                    >
                      {i + 1}
                    </span>
      ...
      <div className="max-w-5xl mx-auto">
        ...
        <div className="mb-8">
          ...
          <p className="text-slate-500 text-sm mt-1">
            Checklist des tâches critiques avant déploiement en production. Réservé aux
            administrateurs.
          </p>
        </div>
          ...
          <div className="flex flex-col sm:flex-row items-start sm:items-center gap-6">
            <div className="flex-1">
              ...
              <p className="text-slate-500 text-sm">
                {okCount} tâche{okCount !== 1 ? 's' : ''} conforme{okCount !== 1 ? 's' : ''} sur{' '}
                {totalTasks}
              </p>
              <div
                className="mt-3 h-2.5 bg-slate-100 rounded-full overflow-hidden"
                role="progressbar"
                aria-valuenow={readinessScore}
                aria-valuemin={0}
                aria-valuemax={100}
                aria-label={`Score de préparation : ${readinessScore}%`}
              >
                <div
            ...
            <div className="flex gap-4 flex-wrap">
              {[
                ...
                <div
                  key={s.label}
                  className={`${s.bg} rounded-xl px-4 py-3 text-center min-w-[80px]`}
                >
        ...
        <div className="bg-amber-50 border border-amber-200 rounded-xl px-5 py-4 mb-8 flex items-start gap-3">
          ...
          <p className="text-amber-800 text-sm leading-relaxed">
            <strong>Accès restreint :</strong> Cette page est réservée aux administrateurs. Les
            informations ci-dessous reflètent l'état de préparation au déploiement en production et
        ...
        <div className="mt-8 p-5 bg-slate-50 border border-slate-200 rounded-xl">
          <p className="text-slate-500 text-xs leading-relaxed">
            <strong className="text-slate-600">Note :</strong> Cette checklist est indicative et
            doit être complétée par une revue de sécurité professionnelle avant tout déploiement en
            production. Consultez un expert en cybersécurité et un juriste spécialisé RGPD pour
